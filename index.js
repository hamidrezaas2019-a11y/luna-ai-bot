...
// ==================== KV FUNCTIONS (با SESSIONS) ====================

async function getUserState(chatId, env) {
  const key = `user:${chatId}`;
  let data = await env.SESSIONS.get(key, 'json');
...
async function saveUserState(chatId, state, env) {
  const key = `user:${chatId}`;
  await env.SESSIONS.put(key, JSON.stringify(state));
}

// ==================== GROUP SETTINGS (با SESSIONS) ====================

async function getGroupSettings(env, groupId) {
  const key = `group:${groupId}`;
  let settings = await env.SESSIONS.get(key, 'json');
  if (!settings) {
    settings = { mode: 'mention', typingIndicator: true, autoDelete: false };
    await env.SESSIONS.put(key, JSON.stringify(settings));
  }
  return settings;
}

async function saveGroupSettings(env, groupId, settings) {
  await env.SESSIONS.put(`group:${groupId}`, JSON.stringify(settings));
}
