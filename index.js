...
// ✅ اصلاح تابع handleStartCommand - حذف کد تکراری
async function handleStartCommand(message: Message, env: Env) {
  const { chat, from } = message;
  if (!from) return;
  ...
  // منطق یکپارچه برای کاربر جدید و قدیمی
  if (session.statistics.totalMessages === 0 && !isGroup) {
    await notifyAdminNewUser(from, env);
    // نمایش انتخاب زبان
  }
...
async function ttsHandler(msgText: string, chatId: number, replyToId: number, env: Env): Promise<boolean> {
  ...
  if (!res.ok) {
    console.log("TTS failed:", res.status);
    return false;
  }
  ...
  const tg = await fetch(
    `https://api.telegram.org/bot${config.TOKEN}/sendVoice`,
    {
      method: 'POST',
      body: form
    }
  ...
  if (!tg.ok) {
    console.log("Telegram sendVoice failed:", await tg.text());
    return false;
  }
...
async function showGroupVIPPanel(chatId: number, messageId: number, env: Env): Promise<void> {
  try {
    ...
    loggerError("showGroupVIPPanel error:", error);
...
// ===== تابع initializeBot =====
async function initializeBot(env, config) {
  logger.info("Starting bot initialization...");
  logger.info("Bot initialized successfully!");
}

// ===== تابع handleUpdate =====
async function handleUpdate(update, env, config) {
  ...
  if (update.message) {
    ...
    const API_URL = `https://api.telegram.org/bot${env.TOKEN}`;
...
// ===== تابع createHealthCheckResponse =====
async function createHealthCheckResponse(env) {
  return new Response(JSON.stringify({
    status: 'ok',
    timestamp: new Date().toISOString(),
    kv: env.SESSIONS ? 'connected' : 'disconnected'
  }), {
    headers: { 'Content-Type': 'application/json' }
  });
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path === "/" || path === "") {
      let result = "SESSIONS type: " + typeof env.SESSIONS;
    ...
    if (request.method === "POST" && path === "/webhook") {
      try {
        ...
        logger.error("Webhook parse failed", error);
...
// ============================================================
// 📦 توابع کمکی اضافی (Helpers) - قبل از export default
// ============================================================
...
// ===== PERSONALITIES =====
const PERSONALITIES = {
  nova: { emoji: "🤖", name_fa: "نوا", name_en: "Nova" },
  lilith: { emoji: "🖤", name_fa: "لیلیت", name_en: "Lilith" },
  cipher: { emoji: "💀", name_fa: "سایفر", name_en: "Cipher" },
  leatherface: { emoji: "🪚", name_fa: "صورت‌چرمی", name_en: "Leatherface" },
  aria: { emoji: "🌙", name_fa: "آریا", name_en: "Aria" },
  jax: { emoji: "🔥", name_fa: "جکس", name_en: "Jax" },
  luna: { emoji: "🧠", name_fa: "لونا", name_en: "Luna" },
  zara: { emoji: "✨", name_fa: "زارا", name_en: "Zara" }
};
...
function createConfig(env) {
  return {
    TOKEN: env.TOKEN,
    GEMINI_KEYS: [env.GEMINI_KEY_1, env.GEMINI_KEY_2, env.GEMINI_KEY_3].filter(Boolean),
    BOT_OWNER_ID: 5989309344
  };
}
...
// ===== تابع sendPersonalityMenu =====
async function sendPersonalityMenu(chatId, messageId, session) {
  const lang = session.language || 'fa';
  const currentPersonality = session.activePersonality || 'nova';
  
  let text = lang === 'fa' 
    ? `🎭 **انتخاب شخصیت**\n\nشخصیت فعلی: **${PERSONALITIES[currentPersonality]?.name_fa || 'نوا'}**\n\nلطفاً شخصیت مورد نظر خود را انتخاب کنید:`
    : `🎭 **Select Personality**\n\nCurrent personality: **${PERSONALITIES[currentPersonality]?.name_en || 'Nova'}**\n\nPlease select your desired personality:`;
...
// ===== تابع initializeBot =====
async function initializeBot(env, cfg) {
  logger.info("Starting bot initialization...");
  logger.info("Bot initialized successfully!");
}
...
// ===== تابع processBroadcastBatch =====
async function processBroadcastBatch(env) {
  logger.info("Processing broadcast batch...");
  // کد broadcast اینجا
}
...
// ============================================================
// 📦 تابع نمایش منوی شخصیت‌ها در Help
// ============================================================
...
async function showPersonalityHelp(cb: CallbackQuery, env: Env): Promise<void> {
  ...
  let text = lang === 'fa'
    ? `🎭 **راهنمای شخصیت‌ها**\n\n` +
      `ربات نوا دارای ۸ شخصیت متفاوت است:\n\n` +
      `🤖 **نوا** - دستیار هوشمند و مودب (پیش‌فرض)\n` +
      `🖤 **لیلیت** - اغواگر و بی‌پروا\n` +
      `💀 **سایفر** - هکر مرموز\n` +
      ...
      `🧠 **لونا** - مغز متفکر\n` +
      `✨ **زارا** - خلاق و هنری\n\n` +
      `💡 برای تغییر از منوی اصلی استفاده کنید.`
    : `🎭 **Personalities Guide**\n\n` +
      `Nova has 8 unique personalities:\n\n` +
      `🤖 **Nova** - Smart and polite (default)\n` +
      `🖤 **Lilith** - Reckless seducer\n` +
      `💀 **Cipher** - Mysterious hacker\n` +
      `🪚 **Leatherface** - Brutal killer\n` +
      `🌙 **Aria** - Rebel philosopher\n` +
      `🔥 **Jax** - Chaos bringer\n` +
      `🧠 **Luna** - Deep thinker\n` +
      `✨ **Zara** - Creative and artistic\n\n` +
...
// ============================================================
// 📦 تابع ذخیره تاریخچه رسانه (تکمیل شده)
// ============================================================
...
// ============================================================
// 📦 تابع فراخوانی SambaNova API (تکمیل شده)
// ============================================================
...
// ============================================================
// 📦 بخش نهایی Worker - تکمیل
// ============================================================

// ادامه بخش scheduled که در بخش قبل ناقص مانده بود:

      // پاکسازی کش‌های حافظه‌ای
      if (sessionCache.size() > 200) {
        // حذف قدیمی‌ترین آیتم‌ها
        logger.info(`🧹 Session cache size: ${sessionCache.size()}, cleaning...`);
      }
      
      if (userBuckets.size > 1000) {
        // پاکسازی bucketهای قدیمی
        const entries = Array.from(userBuckets.entries());
        const now = Date.now();
        let removed = 0;
        
        for (const [userId, bucket] of entries) {
          // اگر بیش از 1 ساعت غیرفعال بوده
        ...
        if (removed > 0) {
          logger.info(`🧹 Removed ${removed} inactive user buckets`);
        }
      }

      // پاکسازی گروه‌های خالی
      for (const [chatId, context] of groupContextCache.entries()) {
        if (context.messages.length === 0 || Date.now() - context.lastCleanup > 3600000) {
          groupContextCache.delete(chatId);
        }
      }
