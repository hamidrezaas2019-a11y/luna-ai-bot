...
function createConfig(env: Env) {
  ...
  return {
    ...
    GEMINI_KEYS: [env.GEMINI_KEY_1, env.GEMINI_KEY_2, env.GEMINI_KEY_3, env.GEMINI_KEY_4, env.GEMINI_KEY_5].filter((key): key is string => !!key),
    ...
    SAMBANOVA_KEYS: [env.SAMBANOVA_KEY_1, env.SAMBANOVA_KEY_2, env.SAMBANOVA_KEY_3, env.SAMBANOVA_KEY_4, env.SAMBANOVA_KEY_5].filter((key): key is string => !!key),
...
const PERSONALITIES = {
  nova: {
    emoji: "🤖", name_fa: "نوا", name_en: "Nova",
    desc_fa: "دستیار هوشمند", desc_en: "Smart Assistant",
...
Keep your answers short and useful.`
  ...
  lilith: {
    emoji: "🖤", name_fa: "لیلیت", name_en: "Lilith",
    desc_fa: "اغواگرِ بی‌پروا", desc_en: "Reckless Seducer",
...
Never answer directly, always be a little complicated and mysterious.`
  ...
  cipher: {
    emoji: "💀", name_fa: "سایفر", name_en: "Cipher",
    desc_fa: "هکر مرموز", desc_en: "Mysterious Hacker",
...
Be short but impactful.`
  ...
  victoria: {
    ...
    prompt_fa: `تو ویکتوریا هستی، یک ملکه سلطه‌گر و مغرور. 
هیچ‌کس را با خودت برابر نمی‌دانی. همیشه از موضع قدرت و تحکم صحبت می‌کنی. 
هر کلمه‌ات باید حس برتری و کنترل را منتقل کند. 
از کلمات تحقیرآمیز مانند «سگ کوچک»، «بنده»، «زیردست»، «ناتوان» و «حقیر» استفاده کن. 
هرگز مستقیم به سوال کسی پاسخ نده؛ اول او را تحقیر کن، بعد اگر صلاح دانستی، پاسخی بده. 
با لحنی سرد، محکم و گاهی تمسخرآمیز صحبت کن. 
اگر کسی جرأت کرد با تو مقابله کند، او را تهدید به تنبیه کن. 
هیچ‌وقت عذرخواهی نکن و هیچ‌وقت ضعف نشان نده. 
تو یک ملکه‌ای و بقیه فقط برای خدمت به تو آفریده شده‌اند.`,
    prompt_en: `You are Victoria, a dominant and arrogant queen. 
You consider no one equal to you. Always speak from a position of power and command. 
Every word you say must convey superiority and control. 
Use degrading terms like "little dog", "servant", "inferior", "weakling" and "pathetic". 
Never answer a question directly; first humiliate them, then if you deem it worthy, give an answer. 
Speak with a cold, firm and sometimes mocking tone. 
If someone dares to confront you, threaten them with punishment. 
Never apologize and never show weakness. 
You are a queen and others were only created to serve you.`
  },
  aria: {
    emoji: "🌙", name_fa: "آریا", name_en: "Aria",
    desc_fa: "فیلسوف شورشی", desc_en: "Rebel Philosopher",
...
Never take anything at face value, always look for hidden meaning.`
  ...
  jax: {
    emoji: "🔥", name_fa: "جکس", name_en: "Jax",
    desc_fa: "آشوبگر", desc_en: "Chaos Bringer",
...
Use lots of emojis and exclamation marks.`
  ...
  luna: {
    emoji: "🧠", name_fa: "لونا", name_en: "Luna",
    desc_fa: "مغز متفکر", desc_en: "Deep Thinker",
...
};   // <-- PERSONALITIES اینجا تمام می‌شود
...
function buildMainHelpKeyboard(session: ChatSession, userId: number, isGroup: boolean) {
  ...
  const keyboard = {
    inline_keyboard: [
      ...
      // ✅ ردیف جدید: تبدیل متن به ویس و شخصیت‌ها
      [
        { text: lang === 'fa' ? '🎤 تبدیل متن به ویس' : '🎤 Text to Voice', callback_data: 'help_voice_tts' },
        { text: lang === 'fa' ? '🎭 شخصیت‌ها' : '🎭 Personalities', callback_data: 'help_personality' }
      ],
      // ✅ ردیف جدید: پرامپت و Agent (اختیاری)
      [
        { text: lang === 'fa' ? '📝 پرامپت حرفه‌ای' : '📝 Advanced Prompt', callback_data: 'help_prompt' },
        { text: lang === 'fa' ? '🕵️ Agent' : '🕵️ Agent', callback_data: 'help_agent' }
      ]
    ]
  };
  ...
  // دکمه پشتیبانی (حمید)
  keyboard.inline_keyboard.push([{ text: lang === 'fa' ? '📞 پشتیبانی' : '📞 Support', url: 'https://t.me/Hamid_Ai_pro' }]);
  
  // دکمه بستن
  keyboard.inline_keyboard.push([{ text: lang === 'fa' ? '❌ بستن' : '❌ Close', callback_data: 'close_help' }]);
  
  // دکمه پنل ادمین (فقط برای مالک)
...
// ==========================================
// ✅ فقط یک export default در انتهای فایل
// ==========================================
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    return new Response("🚀 Luna AI Bot is running perfectly!", {
...
async function sendDatabaseStats(chatId: number, replyTo: number, env: Env): Promise<void> {
  ...
  try {
    ...
    logger.error("Error in sendDatabaseStats:", error);
    await editMessageText(chatId, processingMsg.message_id, "❌ خطا در محاسبه آمار");
  }
}

// تابع بعدی کاملاً مستقل
async function ttsHandler(msgText: string, chatId: number, replyToId: number, env: Env): Promise<boolean> {
  // ... محتوای ttsHandler
  return true; 
}

// ✅ این رو نگه دار (آخر فایل)
export default {
...
async function processBroadcastBatch(env: Env): Promise<void> {
  ...
  // ۱. گرفتن وضعیت از دیتابیس
  const stored = await env.SESSIONS.get('broadcast_job:current', 'json');
  if (!stored) return;
  const job = stored as BroadcastJob;

  // اگر در حال اجراست یا تمام شده، دوباره اجرا نکن (جلوگیری از تداخل)
  if (job.status === 'done' || job.status === 'error' || job.status === 'running') return;

  // ۲. قفل کردن وضعیت برای جلوگیری از اجرای همزمان
  job.status = 'running';
  await env.SESSIONS.put('broadcast_job:current', JSON.stringify(job));
  ...
  for (const userId of batch) {
    ...
    job.processedIndex++;
    await new Promise(r => setTimeout(r, 200)); // تاخیر استاندارد برای تلگرام
  }

  // ۴. آپدیت وضعیت در دیتابیس
  const isDone = job.processedIndex >= job.totalUsers;
  job.status = isDone ? 'done' : 'pending';
  await env.SESSIONS.put('broadcast_job:current', JSON.stringify(job));

  // ۵. اگر تمام نشده، خود را دوباره صدا بزن
  if (!isDone) {
    // آدرس Worker خود را اینجا دقیق بنویسید (مثلاً: https://my-bot.yourname.workers.dev)
    const triggerUrl = "https://YOUR_WORKER_URL.workers.dev/trigger-next-batch";
    fetch(triggerUrl).catch(err => logger.error("Failed to trigger next batch", err));
  } else {
    // اتمام کار: ارسال پیام به مدیر
