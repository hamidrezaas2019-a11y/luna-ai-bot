// ============================================
// @MyNovaChatBot - سورس اصلی نوا (نسخه نهایی یکپارچه)
// شامل تمام اصلاحات: رفع باگ تغییر مدل، تداخل تاریخچه، دستورات کامل
// نسخه اصلاح‌شده نهایی - بدون حذف حتی یک خط از کد اصلی
// ============================================
...
const MODELS = {
  nova: { name: 'NOVA', emoji: '🤖', description: 'سریع و دقیق، پشتیبانی کامل از فارسی، چند رسانه‌ای' },
  luna: { name: 'Luna', emoji: '🧠', description: 'مدل‌های متنوع، قدرتمند در استدلال' },
  zara: { name: 'Zara', emoji: '🎨', description: 'مدل‌های متنوع، خلاقیت بالا، رایگان و نامحدود' }
};
...
const LUNA_MODELS = [
  ...
  'Llama-4-Scout-17B-16E-Instruct', 'Llama-4-Maverick-17B-128E-Instruct', 'o1-preview', 'o3-mini'
];
...
// ========== توابع D1 ==========
async function getUserFromD1(env, telegramId) {
  try {
    ...
    return {
      ...
      currentModel: result.current_model || 'zara',
      language: result.language || 'fa',
      isVIP: result.is_vip === 1,
      ...
      lunaModel: result.luna_model || 'Meta-Llama-3.1-8B-Instruct',
      zaraModel: result.zara_model || 'openai',
      lastImage: null
    };
  } catch (e) {
    console.error('D1 getUserFromD1 error:', e.message);
    return null;
  }
}

async function saveUserToD1(env, user) {
  try {
    await env.DB.prepare(
      `INSERT OR REPLACE INTO users 
       (telegram_id, first_name, last_name, username, current_model, language, is_vip, blocked, last_active, total_messages, luna_model, zara_model, prompts)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      user.id,
      user.firstName || '',
      user.lastName || '',
      user.username || '',
      user.currentModel || 'zara',
      user.language || 'fa',
      user.isVIP ? 1 : 0,
      user.blocked ? 1 : 0,
      user.lastActive || new Date().toISOString(),
      user.totalMessages || 0,
      user.lunaModel || 'Meta-Llama-3.1-8B-Instruct',
      user.zaraModel || 'openai',
      JSON.stringify(user.prompts || DEFAULT_PROMPTS)
    ).run();
  } catch (e) {
    console.error('D1 saveUserToD1 error:', e.message);
  }
}

async function saveConversation(env, userId, model, role, content) {
  try {
    ...
    await env.DB.prepare(
      `INSERT INTO conversations (user_id, model, role, content, created_at)
       VALUES (?, ?, ?, ?, ?)`
    ).bind(
      user.id,
      model,
    ...
    console.error('D1 saveConversation error:', e.message);
  }
}
// ===================================
...
async function getUser(env, userId) {
  ...
  let userData = await env.USERS.get(key, 'json');

  if (!userData) {
    const d1User = await getUserFromD1(env, String(userId));
    if (d1User) {
      userData = d1User;
      await env.USERS.put(key, JSON.stringify(userData));
    }
  }

  if (!userData) {
    userData = {
      id: String(userId), firstName: '', lastName: '', username: '',
      currentModel: 'zara',
      language: 'fa', isVIP: false, blocked: false,
      createdAt: new Date().toISOString(),
      prompts: { ...DEFAULT_PROMPTS },
      history: { nova: [], luna: [], zara: [] },
      limits: { date: '', messages: 0, voiceSent: 0, voiceReceived: 0, images: 0 },
      totalMessages: 0, lastActive: new Date().toISOString(),
      lunaModel: 'Meta-Llama-3.1-8B-Instruct', zaraModel: 'openai', lastImage: null
    };
    await saveUser(env, userId, userData);
    try {
      const current = parseInt(await env.USERS.get('total_users') || '0');
      await env.USERS.put('total_users', String(current + 1));
    } catch {}
  }
...
async function saveUser(env, userId, userData) {
  ...
  await env.USERS.put(key, JSON.stringify(userData));
  try {
    await saveUserToD1(env, userData);
  } catch (e) {
    console.error('saveUser D1 error:', e.message);
  }
}
...
async function getAllUsers(env) {
  ...
  let iterations = 0;
  do {
    const list = await env.USERS.list({ cursor, prefix: 'user:' });
...
async function getAdminState(env, adminId) {
  return await env.USERS.get(`admin_state:${adminId}`, 'json') || null;
}

async function setAdminState(env, adminId, state) {
  if (state === null) await env.USERS.delete(`admin_state:${adminId}`);
  else await env.USERS.put(`admin_state:${adminId}`, JSON.stringify(state), { expirationTtl: 600 });
}

async function getGroupSettings(env, groupId) {
  const key = `group:${groupId}`;
  let settings = await env.USERS.get(key, 'json');
  if (!settings) {
...
async function saveGroupSettings(env, groupId, settings) {
  await env.USERS.put(`group:${groupId}`, JSON.stringify(settings));
}
...
async function addLog(env, entry) {
  try {
    let logs = await env.USERS.get('logs', 'json') || [];
    logs.unshift({ ts: new Date().toLocaleTimeString('fa-IR'), type: entry.type || 'info', msg: entry.message });
    if (logs.length > 200) logs = logs.slice(0, 200);
    await env.USERS.put('logs', JSON.stringify(logs));
  } catch {}
}

async function getLogs(env, filter = 'all') {
  let logs = await env.USERS.get('logs', 'json') || [];
...
async function callGemini(env, messages) {
  ...
  for (const msg of messages) {
    if (msg.role === 'system') { contents.push({ role: 'user', parts: [{ text: msg.content }] }); contents.push({ role: 'model', parts: [{ text: 'باشه' }] }); }
  ...
  for (const key of geminiKeys) {
    try {
      ...
      if (!response.ok) { const errText = await response.text(); lastError = new Error(`Gemini error (${response.status}): ${errText}`); continue; }
      const data = await response.json();
      if (!data.candidates?.[0]?.content?.parts?.[0]?.text) { lastError = new Error('Invalid Gemini response'); continue; }
...
async function callSambaNova(env, messages, model = 'Meta-Llama-3.1-8B-Instruct') {
  const response = await fetch('https://api.sambanova.ai/v1/chat/completions', {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${env.SAMBANOVA_API_KEY}`, 'Content-Type': 'application/json' },
  ...
  for (const m of uniqueModels) {
    try {
      const response = await fetch('https://text.pollinations.ai/', {
        ...
        body: JSON.stringify({ messages, model: m, seed: Math.floor(Math.random() * 99999) })
      });
      if (response.status === 429) { lastError = new Error('429 rate limit'); continue; }
      if (!response.ok) { const errText = await response.text(); lastError = new Error(`Pollinations error (${response.status})`); continue; }
...
async function callAI(env, user, messages) {
  ...
  if (key === 'nova') {
    try {
      ...
      await addLog(env, { type: 'warning', message: `Gemini failed, fallback to Pollinations: ${e.message}` });
      try {
  ...
  if (key === 'luna') {
    ...
    try {
      ...
      await addLog(env, { type: 'warning', message: `SambaNova failed, fallback to Pollinations: ${e.message}` });
      return await callPollinations(env, messages, user.zaraModel || 'openai');
    }
  }
...
async function generateImagesWithThreeModels(env, prompt, seed) {
  ...
  if (env.PRODIA_API_KEY && env.PRODIA_API_KEY !== 'temp') {
    try {
      const prodiaResponse = await fetch('https://api.prodia.com/v1/sd/generate', { method: 'POST', headers: { 'X-Prodia-Key': env.PRODIA_API_KEY, 'Content-Type': 'application/json' }, body: JSON.stringify({ model: 'sdv1_4.ckpt', prompt, steps: 20, seed, width: 1280, height: 1280 }) });
      const prodiaData = await prodiaResponse.json();
      const jobId = prodiaData.job;
      let imageReady = false, attempts = 0;
      while (!imageReady && attempts < 10) { await new Promise(r => setTimeout(r, 1000)); const checkResponse = await fetch(`https://api.prodia.com/v1/job/${jobId}`, { headers: { 'X-Prodia-Key': env.PRODIA_API_KEY } }); if ((await checkResponse.json()).status === 'succeeded') imageReady = true; attempts++; }
      if (imageReady) urls.push(`https://images.prodia.xyz/${jobId}.png`);
    } catch {}
  }
  if (env.OPENAI_API_KEY && env.OPENAI_API_KEY !== 'temp') {
    try {
      const r = await fetch('https://api.openai.com/v1/images/generations', { method: 'POST', headers: { 'Authorization': `Bearer ${env.OPENAI_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt, n: 1, size: '1024x1024' }) });
...
// *** اصلاح: تابع isGroupAdmin فقط true/false برمی‌گردونه ***
...
function setupBot(bot, env) {
  ...
  bot.use(async (ctx, next) => {
    ...
    const user = await getUser(env, ctx.from.id);
  ...
  async function sendMainMenu(ctx) {
    ...
    // اطمینان از وجود currentModel معتبر
    let currentModel = user.currentModel;
    if (!currentModel || !MODELS[currentModel]) {
      currentModel = 'nova'; // مدل پیش‌فرض
    }
    
    // دریافت اطلاعات مدل، با fallback دستی
  ...
  bot.command('/start', async (ctx) => {
    ...
    await saveUser(ctx.env, ctx.from.id, user);
  ...
  bot.callbackQuery('keys_refresh', async (ctx) => {
    ...
    if (geminiKey && geminiKey !== 'temp') {
      try {
        ...
        if (testRes.ok) {
          ...
          results.gemini.push({ key: shortKey, status: errData?.error?.code === 429 ? 'ratelimit' : 'error', code: testRes.status });
        }
      } catch {}
    }
    ...
    if (sambanovaKey && sambanovaKey !== 'temp') {
      try {
        ...
        results.sambanova.push({ key: shortKey, status: testRes.ok ? 'ok' : 'error', code: testRes.status });
      } catch {}
    }
    ...
    text += `🤖 Gemini (نوا):\n`;
    results.gemini.length === 0 ? text += `❌ کلید تنظیم نشده\n` :
      results.gemini.forEach((r, i) => {
        text += `${i + 1}. ${r.key} ${r.status === 'ok' ? '🟢 سالم (OK)' : '🔴 مسدود/خطا'}\n`;
      });

    text += `\n🧠 SambaNova (لونا):\n`;
    results.sambanova.length === 0 ? text += `⚠️ کلید تنظیم نشده\n` :
      results.sambanova.forEach((r, i) => {
        text += `${i + 1}. ${r.key} ${r.status === 'ok' ? '🟢 سالم (OK)' : '🔴 خطا'}\n`;
      });
  ...
  bot.callbackQuery('group_settings_menu', async (ctx) => {
    ...
    const s = await getGroupSettings(ctx.env, ctx.chat.id);
  ...
  bot.callbackQuery('gmode_all', async (ctx) => {
    ...
    const s = await getGroupSettings(ctx.env, ctx.chat.id);
    s.mode = 'all'; await saveGroupSettings(ctx.env, ctx.chat.id, s);
  ...
  bot.callbackQuery('gmode_mention', async (ctx) => {
    ...
    const s = await getGroupSettings(ctx.env, ctx.chat.id);
    s.mode = 'mention'; await saveGroupSettings(ctx.env, ctx.chat.id, s);
  ...
  bot.callbackQuery('gmode_smart', async (ctx) => {
    ...
    const s = await getGroupSettings(ctx.env, ctx.chat.id);
    s.mode = 'smart'; await saveGroupSettings(ctx.env, ctx.chat.id, s);
  ...
  bot.callbackQuery('gtoggle_typing', async (ctx) => {
    ...
    const s = await getGroupSettings(ctx.env, ctx.chat.id);
    s.typingIndicator = !s.typingIndicator; await saveGroupSettings(ctx.env, ctx.chat.id, s);
  ...
  bot.command('/setprompt', async (ctx) => {
    ...
    await saveUser(ctx.env, ctx.from.id, user);
    await ctx.reply(
      `✅ *پرامپت ${MODELS[key].name} تنظیم و اعمال شد*\n\n` +
      `بدون نیاز به /new از الان فعال است!\n\n` +
      `🧠 حافظه مکالمه پاک شد!\n\n` +
      `مدل فعال: ${MODELS[user.currentModel].name}\n\n` +
  ...
  bot.command('/img', async (ctx) => {
    ...
    if (!checkLimit(user, 'images')) { await saveUser(ctx.env, ctx.from.id, user); return ctx.reply(t(user, '❌ به سقف تصویر روزانه (۵) رسیدید.', '❌ Daily image limit reached.')); }
    ...
    try {
      ...
      await addLog(ctx.env, { type: 'error', message: `img command error: ${e.message}` });
  ...
  bot.command('/edit', async (ctx) => {
    ...
    if (!checkLimit(user, 'images')) { await saveUser(ctx.env, ctx.from.id, user); return ctx.reply('❌ سقف تصویر.'); }
    await ctx.api.sendChatAction(ctx.chat.id, 'upload_photo');
    try {
  ...
  bot.on('message:text', async (ctx) => {
    ...
    // *** اصلاح: چک کردن وجود message_id قبل از ری‌اکشن ***
    if (ctx.message?.message_id && ctx.chat?.id) {
      let reaction = '👍';
      const t2 = text.toLowerCase();
      if (t2.includes('😂') || t2.includes('خنده') || t2.includes('جوک') || t2.includes('باحال') || t2.includes('خندیدم') || t2.includes('😁')) reaction = '🤣';
      else if (t2.includes('❤️') || t2.includes('عشق') || t2.includes('دوست دارم') || t2.includes('مرسی') || t2.includes('ممنون')) reaction = '❤️';
      else if (t2.includes('وای') || t2.includes('عالی') || t2.includes('باورم') || t2.includes('جدی') || t2.includes('😮') || t2.includes('واقعا')) reaction = '😮';
      else if (t2.includes('؟') || t2.includes('?') || t2.includes('چرا') || t2.includes('چطور') || t2.includes('چیه') || t2.includes('کیه')) reaction = '🤔';
      else if (t2.includes('آتیش') || t2.includes('فایر') || t2.includes('🔥') || t2.includes('خفن') || t2.includes('داغ')) reaction = '🔥';
      else if (t2.includes('غمگین') || t2.includes('ناراحت') || t2.includes('😢') || t2.includes('گریه') || t2.includes('افسرده')) reaction = '🥺';
      else if (t2.includes('اوکی') || t2.includes('باشه') || t2.includes('چشم') || t2.includes('آره') || t2.includes('بله')) reaction = '👌';
    ...
    if (String(userId) === String(ctx.env.ADMIN_ID)) {
      const adminState = await getAdminState(ctx.env, userId);
      if (adminState) {
        await setAdminState(ctx.env, userId, null);
        if (adminState.action === 'broadcast') {
          const users = await getAllUsers(ctx.env);
          let sent = 0, failed = 0;
        ...
        if (adminState.action === 'add_vip') {
          const target = await getUser(ctx.env, text.trim()); target.isVIP = true;
          await saveUser(ctx.env, text.trim(), target);
        ...
        if (adminState.action === 'block_user') {
          const target = await getUser(ctx.env, text.trim()); target.blocked = true;
          await saveUser(ctx.env, text.trim(), target);
          await ctx.reply(`🚫 کاربر ${text.trim()} مسدود شد.`); return;
        }
        if (adminState.action === 'unblock_user') {
          const target = await getUser(ctx.env, text.trim()); target.blocked = false;
          await saveUser(ctx.env, text.trim(), target);
          await ctx.reply(`✅ کاربر ${text.trim()} رفع مسدود شد.`); return;
        }
        if (adminState.action === 'change_user_model') {
          ...
          const target = await getUser(ctx.env, targetId);
          target.currentModel = newModel;
          await saveUser(ctx.env, targetId, target);
    ...
    if (!checkLimit(user, 'messages')) {
      await saveUser(ctx.env, userId, user);
    ...
    try {
      ...
      try {
        await saveConversation(env, userId, modelKey, 'user', text);
        await saveConversation(env, userId, modelKey, 'assistant', reply);
      } catch (e) {
        console.error('saveConversation error:', e.message);
      }

      await addLog(ctx.env, { type: 'info', message: `✅ Success Model: ${modelKey}` });
      ...
      await addLog(ctx.env, { type: 'error', message: `API Call Failed: ${e.message}` });
  ...
  bot.on('message:voice', async (ctx) => {
    ...
    const userId = ctx.from.id;
    const user = ctx.user;
    if (!checkLimit(user, 'voiceReceived')) { await saveUser(ctx.env, userId, user); return ctx.reply(t(user, '❌ به سقف ویس دریافتی روزانه (۱۰) رسیدید.', '❌ Daily voice limit reached.')); }
    ...
    try {
      ...
      await addLog(ctx.env, { type: 'error', message: `Voice handler error: ${e.message}` });
  ...
  bot.on('message:photo', async (ctx) => {
    ...
    try {
      ...
      await addLog(ctx.env, { type: 'error', message: `Photo handler error: ${e.message}` });
  ...
  bot.on('message:animation', async (ctx) => {
    ...
    try {
      ...
      await addLog(ctx.env, { type: 'error', message: `Animation handler error: ${e.message}` });
  ...
  bot.on('message:video', async (ctx) => {
    ...
    try {
      ...
      await addLog(ctx.env, { type: 'error', message: `Video handler error: ${e.message}` });
  ...
  async function showAdminUsers(ctx, page, sortBy) {
    ...
    const allUsers = await getAllUsers(ctx.env);
  ...
  bot.callbackQuery('adm_keys_btn', async (ctx) => {
    ...
    const safeFetch = async (url, opts) => {
      try { return await Promise.race([fetch(url, opts), timeout(5000)]); }
      catch { return null; }
    };
    ...
    if (geminiKey && geminiKey !== 'temp') {
      const shortKey = geminiKey.substring(0, 8) + '...' + geminiKey.slice(-4);
      const testRes = await safeFetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=${geminiKey}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ contents: [{ parts: [{ text: 'hi' }] }] })
      });
      if (!testRes) results.gemini = { key: shortKey, status: 'timeout' };
      else if (testRes.ok) results.gemini = { key: shortKey, status: 'ok' };
      else {
        const errData = await testRes.json().catch(() => ({}));
    ...
    if (sambanovaKey && sambanovaKey !== 'temp') {
      const shortKey = sambanovaKey.substring(0, 8) + '...' + sambanovaKey.slice(-4);
      const testRes = await safeFetch('https://api.sambanova.ai/v1/chat/completions', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${sambanovaKey}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ model: 'Meta-Llama-3.1-8B-Instruct', messages: [{ role: 'user', content: 'hi' }], max_tokens: 5 })
      });
      if (!testRes) results.sambanova = { key: shortKey, status: 'timeout' };
      else results.sambanova = { key: shortKey, status: testRes.ok ? 'ok' : 'error', code: testRes.status };
    }

    const pollRes = await safeFetch('https://text.pollinations.ai/', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ messages: [{ role: 'user', content: 'hi' }], model: 'openai' })
    });
    results.pollinations = !pollRes ? 'timeout' : pollRes.ok ? 'ok' : 'error';
...
`;
    else text += `${results.gemini.key} ${results.gemini.status === 'ok' ? '🟢 سالم' : results.gemini.status === 'ratelimit' ? '🔴 rate limit' : results.gemini.status === 'timeout' ? '🟡 timeout' : `🔴 خطا (${results.gemini.code})`}
`;
...
`;
    else text += `${results.sambanova.key} ${results.sambanova.status === 'ok' ? '🟢 سالم' : results.sambanova.status === 'timeout' ? '🟡 timeout' : `🔴 خطا (${results.sambanova.code})`}
`;
...
` : results.pollinations === 'timeout' ? `🟡 timeout
` : `🔴 خطا
`;
    text += `
⏰ زمان تست: ${now}`;
  ...
  bot.callbackQuery('adm_broadcast_btn', async (ctx) => {
    if (String(ctx.from.id) !== String(ctx.env.ADMIN_ID)) return ctx.answerCallbackQuery('⛔');
    await setAdminState(ctx.env, ctx.from.id, { action: 'broadcast' });
  ...
  bot.callbackQuery('adm_add_vip_btn', async (ctx) => {
    if (String(ctx.from.id) !== String(ctx.env.ADMIN_ID)) return ctx.answerCallbackQuery('⛔');
    await setAdminState(ctx.env, ctx.from.id, { action: 'add_vip' });
  ...
  bot.callbackQuery('adm_change_model_btn', async (ctx) => {
    if (String(ctx.from.id) !== String(ctx.env.ADMIN_ID)) return ctx.answerCallbackQuery('⛔');
    await setAdminState(ctx.env, ctx.from.id, { action: 'change_user_model' });
    await ctx.editMessageText(
      `🔄 *تغییر مدل کاربر*\n\nفرمت: \`آیدی مدل\`\n\nمثال:\n\`123456789 nova\`\n\`123456789 luna\`\n\`123456789 zara\`\n\nمدل‌های موجود:\n🤖 nova | 🎨 luna | 🔬 zara`,
  ...
  bot.callbackQuery('adm_ask_block', async (ctx) => {
    if (String(ctx.from.id) !== String(ctx.env.ADMIN_ID)) return ctx.answerCallbackQuery('⛔');
    await setAdminState(ctx.env, ctx.from.id, { action: 'block_user' });
  ...
  bot.callbackQuery('adm_ask_unblock', async (ctx) => {
    if (String(ctx.from.id) !== String(ctx.env.ADMIN_ID)) return ctx.answerCallbackQuery('⛔');
    await setAdminState(ctx.env, ctx.from.id, { action: 'unblock_user' });
  ...
  bot.callbackQuery('adm_cancel', async (ctx) => {
    if (String(ctx.from.id) !== String(ctx.env.ADMIN_ID)) return ctx.answerCallbackQuery('⛔');
    await setAdminState(ctx.env, ctx.from.id, null);
    await showAdminUsers(ctx, 1, 'newest');
  ...
  bot.callbackQuery('adm_rebuild_do', async (ctx) => {
    ...
    try {
      ...
      for (const u of users) {
        ...
        if (changed) { await saveUser(ctx.env, u.id, u); fixed++; }
      }
      await ctx.editMessageText(
  ...
  bot.callbackQuery('adm_clearlogs', async (ctx) => {
    if (String(ctx.from.id) !== String(ctx.env.ADMIN_ID)) return ctx.answerCallbackQuery('⛔');
    await ctx.env.USERS.put('logs', JSON.stringify([]));
  ...
  async function showLogs(ctx, filter) {
    const isAdmin = String(ctx.from.id) === String(ctx.env.ADMIN_ID);
    if (!isAdmin) return;
    const allLogs = await getLogs(ctx.env, 'all');
...
`;
  ...
  bot.callbackQuery('log_download', async (ctx) => {
    ...
    await ctx.answerCallbackQuery('⏳ در حال آماده‌سازی...');
    try {
      const logs = await getLogs(ctx.env, 'all');
  ...
  bot.command('/unvip', async (ctx) => {
    ...
    const target = await getUser(ctx.env, targetId); target.isVIP = false;
    await saveUser(ctx.env, targetId, target);
  ...
  bot.command('/block', async (ctx) => {
    ...
    const target = await getUser(ctx.env, targetId); target.blocked = true;
    await saveUser(ctx.env, targetId, target);
  ...
  bot.command('/unblock', async (ctx) => {
    ...
    const target = await getUser(ctx.env, targetId); target.blocked = false;
    await saveUser(ctx.env, targetId, target);
  ...
  bot.command('/stats', async (ctx) => {
    ...
    await ctx.reply(`📊 *آمار ربات NOVA*\n\n👥 کل کاربران: ${users.length}\n💎 VIP: ${vip}\n🚫 مسدود: ${blocked}\n💬 کل پیام‌ها: ${totalMsgs}`, { parse_mode: 'Markdown' });
  });
...
export default {
  async fetch(request, env) {
    ...
    return new Response('ربات NOVA - فعال ✅', { status: 200, headers: { 'Content-Type': 'text/html; charset=utf-8' } });
  }
};
