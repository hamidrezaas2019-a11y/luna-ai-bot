...
// ✅ تنها یک بار تعریف کن
// ❌ این رو پاک کن:
export async function ttsHandler(msgText: string, chatId: number, replyToId: number, env: Env): Promise<boolean> {
  // ... محتوای ttsHandler
  return true; 
}
...
// ✅ جایگزین کن با این:
async function ttsHandler(msgText: string, chatId: number, replyToId: number, env: Env): Promise<boolean> {
  // ... محتوای ttsHandler (همون کد قبلی)
  return true; 
}

// ✅ فقط این یک export default بمونه:
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    // ... کل کد fetch که قبلاً داشتی
  },
