...
// ============================================================
// 🎭 PERSONALITY MENU - نسخه نهایی مثل پنل اصلی
// ============================================================
...
async function showPersonalityMenu(chatId: number, messageId: number, session: ChatSession): Promise<void> {
  ...
  const current = session.activePersonality || 'nova';
  const p = PERSONALITIES[current];
  
  const name = lang === 'fa' ? p.name_fa : p.name_en;
  const desc = lang === 'fa' ? p.desc_fa : p.desc_en;
  
  // لیست شخصیت‌ها با ایموجی و اسم
  const list = Object.keys(PERSONALITIES).map(key => {
    const item = PERSONALITIES[key];
    const label = lang === 'fa' ? item.name_fa : item.name_en;
    const check = key === current ? '✅' : '⬜';
    return `${check} ${item.emoji} ${label}`;
  }).join('\n ');
...
از بین **${Object.keys(PERSONALITIES).length} شخصیت** زیر یکی رو انتخاب کن:

┌────────────────────────┐
 ${list}
└────────────────────────┘

💡 هر شخصیت لحن و رفتار متفاوتی داره.`
    : `🎭 **Nova Personalities**
...
Choose from **${Object.keys(PERSONALITIES).length} personalities**:

┌────────────────────────┐
 ${list}
└────────────────────────┘

💡 Each personality has a different tone and behavior.`;
  ...
  Object.keys(PERSONALITIES).forEach(key => {
    ...
    const check = key === current ? '✅ ' : '';
    row.push(
      { text: `${check}${item.emoji} ${label}`, callback_data: `set_personality_${key}` },
      { text: 'ℹ️', callback_data: `info_${key}` }
    );
  ...
  keyboard.inline_keyboard.push([
    { text: '✏️ پرامپت دستی', callback_data: 'custom_prompt_menu' },
