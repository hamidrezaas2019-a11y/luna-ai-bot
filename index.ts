...
function createConfig(env: Env) {
  ...
  return {
    TOKEN: env.TOKEN,
    BOT_OWNER_ID: env.BOT_OWNER_ID ? parseInt(env.BOT_OWNER_ID, 10) : 0,
...
async function sendDatabaseStats(chatId: number, replyTo: number, env: Env): Promise<void> {
  ...
  try {
    ...
    // ============================================
// 📦 مرحله 1: همه توابع (بدون export) - فقط یک بار
// ============================================
...
// ✅ _processAIRequestInternal (با بدنه کامل)
...
// ============================================
// همه کدهای ربات اینجا قرار دارند (هزاران خط)
// شامل: interface ها، const ها، function ها، class ها و ...
// ============================================

// ✅ فقط یک بار در انتهای فایل:
export default {
    ...
    try {
      ...
      if (request.method === "POST" && path === "/webhook") {
        try {
          ...
          ctx.waitUntil(handleUpdate(update, env, config));
...
// ❌ هیچ کد دیگری بعد از این ننویسید!
// آخرین آکولاد باید همینجا بسته شود
