...
const AGENT_TOOLS = {
  generate_image: { desc: " ", params: { prompt: "string" } },
  search_images: { desc: "   ", params: { query: "string" } }
};
...
function detectErrorType(error: Error): ErrorType {
  ...
  if (msg.includes('expired') || msg.includes('')) return ErrorType.AUTH;
  if (msg.includes('leaked') || msg.includes(' ')) return ErrorType.AUTH;
  if (msg.includes('quota') || msg.includes('rate limit') || msg.includes('429') || msg.includes('')) return ErrorType.QUOTA;
  if (msg.includes('safety') || msg.includes('blocked') || msg.includes('') || msg.includes('content filter')) return ErrorType.BLOCKED;
  if (msg.includes('401') || msg.includes('403') || msg.includes('unauthorized') || msg.includes('')) return ErrorType.AUTH;
  if (msg.includes('empty') || msg.includes('') || msg.includes('no content')) return ErrorType.EMPTY;
  if (msg.includes('timeout') || msg.includes('aborted') || msg.includes('')) return ErrorType.TIMEOUT;
  ...
  if (msg.includes('fetch') || msg.includes('network') || msg.includes('') || msg.includes('failed to fetch')) return ErrorType.NETWORK;
  if (msg.includes('file') || msg.includes('download') || msg.includes('')) return ErrorType.FILE;
  if (msg.includes('voice') || msg.includes('transcribe') || msg.includes('')) return ErrorType.VOICE;
  if (msg.includes('image') || msg.includes('photo') || msg.includes('')) return ErrorType.IMAGE;
...
function formatUserFriendlyErrorNew(error: Error, lang: 'fa' | 'en' = 'fa'): ErrorInfo {
  ...
  const errorMap = {
    [ErrorType.TIMEOUT]: { icon: '', msg: translations.err_timeout || '   ' },
    [ErrorType.NETWORK]: { icon: '', msg: translations.err_network || '   ' },
    [ErrorType.QUOTA]:   { icon: '', msg: translations.err_quota || ' ' },
    [ErrorType.BLOCKED]: { icon: '', msg: translations.err_blocked || '  ' },
    [ErrorType.AUTH]:    { icon: '', msg: translations.err_auth || '  ' },
    [ErrorType.EMPTY]:   { icon: '', msg: translations.err_empty || '   ' },
    [ErrorType.SERVER]:  { icon: '', msg: translations.err_network || ' ' },
    [ErrorType.VOICE]:   { icon: '', msg: translations.err_voice || '   ' },
    [ErrorType.IMAGE]:   { icon: '', msg: translations.err_image || '   ' },
    [ErrorType.FILE]:    { icon: '', msg: translations.err_network || '  ' },
    [ErrorType.UNKNOWN]: { icon: '', msg: translations.err_unknown || ' ' }
  };
  const info = errorMap[errorType] || errorMap[ErrorType.UNKNOWN];
  return {
    type: errorType,
    icon: info.icon,
    title: translations.err_title || '',
...
const TRANSLATIONS = {
  fa: {
    engine_gemini: '', engine_sambanova: '', engine_pollinations: '',
    loading: '   ...', processing: '   ...', typing: '  ...',
    prompt_title: ' **  **', prompt_current: ' :', prompt_default: '',
    prompt_guide: '  : `/setprompt []  `', prompt_reset: '', prompt_show: '  ',
    prompt_manage: '  ',
    system_prompt: " {botName}       .        .  : {date}",
    system_prompt_group: " {botName} .     .     .",
    img_limit: '     .', img_start: ' **  ...**', img_translating: ' **  ...**',
    img_processing: '     {count} ...', img_failed: ' **   .**', img_success: ' ** .**',
    img_help: ' ** **\n\n: `/img []`\n: `/img    `', search_attribution: '\n\n : Pixabay.com',
    btn_settings: ' ', btn_back: ' ', btn_select_model: '  ', btn_prompt: ' () ',
    btn_help: ' ', btn_close: ' ', btn_refresh: ' ', btn_retry: '  ', btn_confirm: '   ',
    btn_cancel: ' ', btn_prev: ' ', btn_next: ' ', err_title: '', err_quota: '     .',
    err_auth: '    (API Key).', err_network: '      .', err_timeout: '   .',
    err_blocked: '       .', err_empty: '  .', err_voice: '   .',
    err_image: '     .', err_unknown: '    .', err_vip_only: '     VIP .',
    err_format: ' ** **', err_empty_prompt: '    .', err_prompt_toolong: '    .',
    err_engine_invalid: '  . : `nova`, `luna`, `arya`, `zara`', err_vip_prompt: ' ** **\n\n     VIP  .',
    err_config_missing: '  Cloudflare   .', active_model_title: ' ** {name}**', active_model_keys: ' **:** {count}',
    active_model_static_desc: ' {name}        .', active_model_current: ' ** :** {name}', active_model_key_idx: ' ** API:** {index}/{total}',
    active_model_count: ' ** :** {count}', active_model_guide: '        ', model_select_title: ' **  {name}**',
    model_total_count: '  : {count} ', model_last_update: '  : {time}', model_page_info: '  {page}  {total}', model_not_found: ' **   {name}  **',
    search_searching: ' **   "{query}"...**\n\n   ', search_results: ' {caption}\n\n {count}   ', search_failed: ' **  **',
    search_guide: ' :\n�     \n�    \n�     ', search_link_fallback: '       :\n\n{link}\n\n {count}   ',
    search_no_results: '   .     .', search_long_query: '    .  100 .', search_usage: ': `/search []`', search_quota_exceeded: '   .',
    admin_view_memory: '  ', admin_reset_memory: '  ', admin_memory_title: ' **  {name}**', admin_memory_empty: ' **  **',
    admin_memory_confirm_reset: ' **  **\n\n      !', admin_memory_reset_success: ' **  **',
    welcome_private: ` ** {name} !**\n\n   ****  -    !\n\n   : ** **\n\n ** :**\n **  :**     (  )\n ** :**     !\n ** :**         .\n ** :**     .\n\n **    :**`,
    welcome_group: ` **    {name}!**\n\n ****  .\n          .\n\n     ****     **** .`,
    help_text: ` **  **\n\n **:**       .\n\n **:**\n�  : \`/img   \`\n� : \`/search \`\n\n **:**\n� /model -   \n� /new -     \n� /prompt -   \n� /language -  `,
  },
  en: {
    engine_gemini: 'Nova', engine_sambanova: 'Luna', engine_pollinations: 'Zara',
    loading: ' Please wait...', processing: ' Processing...', typing: 'typing...',
    prompt_title: ' **Custom Prompt Settings**', prompt_current: 'Current Prompts:', prompt_default: 'Default',
    prompt_guide: ' To set: `/setprompt [engine] your text`', prompt_reset: 'Reset', prompt_show: 'Show Prompts ',
    ...
    img_limit: ' Daily limit exceeded.', img_start: ' **Starting image generation...**', img_translating: ' **Translating...**',
    ...
    btn_settings: 'Settings ', btn_back: 'Back ', btn_select_model: ' Select Model', btn_prompt: 'Prompt (Persona) ',
    btn_help: 'Help ', btn_close: 'Close ', btn_refresh: 'Refresh ', btn_retry: ' Retry', btn_confirm: ' Yes, confirm',
    btn_cancel: ' Cancel', btn_prev: ' Previous', btn_next: 'Next ', err_title: 'Error', err_quota: 'Quota exceeded for this model.',
    ...
    err_image: 'Image generation failed.', err_unknown: 'An unexpected error occurred.', err_vip_only: ' This feature is for VIP users only.',
    err_format: ' **Invalid Format**', err_engine_invalid: ' Invalid Engine. Engines: `nova`, `luna`, `arya`, `zara`',
    err_vip_prompt: ' **Restricted Access**\n\nCustom prompts are for VIP users only.', err_empty_prompt: ' Prompt cannot be empty.',
    err_prompt_toolong: ' Prompt is too long.', err_config_missing: ' Cloudflare config missing.',
    active_model_title: ' **{name} Settings**', active_model_keys: ' **Keys:** {count}', active_model_static_desc: ' {name} uses a stable static model.',
    ...
    search_searching: ' **Searching for "{query}"...**\n\n Please wait', search_results: ' {caption}\n\n {count} images found', search_failed: ' **Search Failed**',
    search_guide: ' Tips:\n� Use simpler keywords\n� Try in English\n� Try again later', search_link_fallback: ' Could not send image directly, here is the link:\n\n{link}\n\n {count} images found',
    ...
    search_quota_exceeded: 'Google quota exceeded.', admin_view_memory: ' View Memory', admin_reset_memory: ' Reset Memory',
    admin_memory_title: ' **User Memory: {name}**', admin_memory_empty: ' **Memory is empty**', admin_memory_confirm_reset: ' **Confirm Memory Reset**\n\nAre you sure? This cannot be undone!',
...
const MODEL_META = {
  gemini: { emoji: "", fa: "", en: "Nova", badge_fa: "  ", badge_en: "Fast & accurate" },
  sambanova: { emoji: "", fa: "", en: "Luna", badge_fa: "  ", badge_en: "Powerful & deep" },
  pollinations: { emoji: "", fa: "", en: "Zara", badge_fa: "  ", badge_en: "Creative & free" }
} as const;
...
function buildModelSelectionKeyboard(session: ChatSession) {
  ...
  return {
    inline_keyboard: [
      ...
      [ btn('pollinations')             ],
      [
        createInlineButton(lang === 'fa' ? '  ' : ' Model Settings', 'active_model_settings'),
        createInlineButton(lang === 'fa' ? ' '        : ' Persona',        'custom_prompt_menu'   ),
      ],
      [ createInlineButton(lang === 'fa' ? ' ' : ' Back', 'open_help') ],
    ]
  };
}
...
async function _saveSingleSession(session: ChatSession, env: Env): Promise<void> {
  ...
  if (jsonStr.length > 2 * 1024 * 1024) {
    logger.warn(` Session ${session.id} too large: ${Math.round(jsonStr.length/1024)}KB`);
    ...
    logger.info(` Compressed session size to ${Math.round(jsonStr.length/1024)}KB`);
  }
...
function getVIPUpgradeKeyboard() {
  return {
    inline_keyboard: [
      [
        { text: "  VIP ", url: "https://t.me/Hamid_Ai_pro" }
      ]
    ]
  };
}
...
  : ${dateStr}
  : ${userName}
${userMemory?.topics?.length > 0 ? `  : ${userMemory.topics.slice(0, 3).join(', ')}` : ''}
${isGroup && groupAnalysis ? `  : ${groupAnalysis}` : ''}`;
  }
...
async function ensurePollinationsModels(env: Env): Promise<void> {
  ...
  try {
    ...
    if (cache.models.length === 0) {
      logger.warn(" API returned 0 models, using fallback");
  ...
  if (selectedModel.type === 'image' || selectedModel.id.includes('flux') || selectedModel.id.includes('turbo')) {
    ...
    try {
        ...
        if (!imageResponse.ok) {
            ...
            if (imageResponse.status === 429) throw new Error("        .");
            if (imageResponse.status === 401) throw new Error(" API   .");
            throw new Error(`Pollinations Image Error: ${imageResponse.status} - ${err.substring(0, 50)}`);
        }

        const arrayBuffer = await imageResponse.arrayBuffer();
        if (arrayBuffer.byteLength < 1000) throw new Error("   .");
...
async function getOrCreateSession(chat: Chat, user: User, env: Env): Promise<ChatSession> {
  ...
  if (isDeleted && sessionCache.get(cacheKey)) {
    ...
    logger.info(` Cleared deleted session ${chat.id} from cache`);
  }
...
function hydrateSession(stored: any, chat: Chat, user: User): ChatSession {
  ...
  if (!session.statistics) {
    ...
    logger.warn(` Reconstructed missing statistics for session ${session.id}`);
  }
...
function formatUserStatistics(users: UserStatistics[]): string {
  ...
  let text = ` **  **\n\n`;
  text += ` ** :** ${totalUsers}\n`;
  text += ` **VIP:** ${vipUsers} |  **:** ${totalUsers - vipUsers}\n`;
  text += ` ** :** ${activeToday}\n\n`;
  
  text += ` ** :**\n`;
  text += ` : ${totalMessages}\n`;
  text += ` : ${engineCounts.gemini}\n`;
  text += ` : ${engineCounts.sambanova}\n`;
  text += ` : ${engineCounts.pollinations}\n\n`;
  ...
  if (mostPopularEngine[1] > 0) {
    ...
    text += ` ** :** ${engLabel} (${mostPopularEngine[1]} )`;  
  }
  
  text += `\n\n`;
  text += ` **  (${Math.min(10, users.length)}  ):**\n\n`;
  ...
  users.slice(0, 10).forEach((user, index) => {
    ...
    text += ` **:** ${user.statistics.totalMessages || 0} | `;
    text += ` ${user.statistics.geminiMessages || 0} | `;
    text += ` ${user.statistics.sambanovaMessages || 0} | `;
    text += ` ${user.statistics.pollinationsMessages || 0}\n`;
    
    text += ` ${user.statistics.voicesReceived || 0} \n`;
    
    text += ` : ${firstUsed} |  : ${lastSeen}\n`;
    
    if (!user.vipStatus) {
      text += ` **:** `;
      text += `${user.dailyLimits.messages || 0}/50  | `;
      text += `${user.dailyLimits.voicesSent || 0}/5   | `;
      text += `${user.dailyLimits.voicesReceived || 0}/10   | `;
    }
...
function formatDetailedUserStats(user: UserStatistics): string {
  ...
  const calculateUsageDuration = (): string => {
    ...
    if (firstUsed === 0 || lastSeen === 0) return '';
  ...
  const getFavoriteEngine = (): { name: string; count: number; percentage: number } => {
    ...
    return {
      name: TRANSLATIONS.fa[`engine_${favorite.key}`] || '',
  ...
  const safeUserId = escapeMarkdown(String(user.userId || ''));
  ...
  text += `� : @${safeUsername}\n`;
  text += `� : \`${safeUserId}\`\n`;
  text += `� : ${user.vipStatus ? ' VIP' : ' '}\n\n`;
  
  text += `${activity.emoji} ** :** ${activity.status}\n`;
  text += `�  : ${usageDuration}\n\n`;
  
  const stats = user.statistics || {};
  text += ` ** :**\n`;
  text += `� **:** ${stats.totalMessages || 0}\n`;
  text += `    : ${stats.geminiMessages || 0}\n`;
  text += `    : ${stats.sambanovaMessages || 0}\n`;
  text += `    : ${stats.pollinationsMessages || 0}\n`;
  text += `\n ** :** ${favorite.name} (${favorite.percentage}%)\n\n`;
  
  text += ` ** :**\n`;
  text += `�   : ${stats.voicesReceived || 0}\n\n`;
  
  if (!user.vipStatus) {
    text += ` ** :**\n`;
    const limits = user.dailyLimits || { messages: 0, voicesSent: 0, voicesReceived: 0, imagesGenerated: 0 };
    text += `�  : ${limits.messages || 0}/100\n`;
    text += `�  : ${limits.voicesSent || 0}/10\n`;
    text += `�  : ${limits.imagesGenerated || 0}/5\n\n`;
  } else {
    text += ` ** VIP -  **\n\n`;
  }
  
  text += ` **:**\n`;
  text += `�  : ${formatSafeDate(stats.firstUsed)}\n`;
  text += `�  : ${formatSafeDate(stats.lastSeen)}\n`;
...
async function handleKeysCommand(chatId: number, messageId: number | undefined, env: Env, isEdit = false) {
  ...
  statusText += ` **Gemini () - ${config.GEMINI_KEYS.length} :**\n`;
  await editMessageText(chatId, currentMsgId!, statusText + `>    ...`);
  ...
  for (let i = 0; i < config.GEMINI_KEYS.length; i++) {
    ...
    if (unlockTime && now < unlockTime) {
      ...
      statusText += `  ${i + 1}. \`${maskedKey}\`   (   ${hoursLeft}  )\n`;
      continue;
    }
  ...
  try {
    ...
    if (zaraRes.ok) {
      statusText += `  ${config.POLLINATIONS_KEY ? '     ' : '     '}\n`;
    } else {
      if (zaraRes.status === 429) {
        ...
        statusText += `    (${zaraRes.status})\n`;
      }
    }
  } catch (error) {
      const rawErr = getRawError(error);
      statusText += `   : ${rawErr.substring(0, 80)}\n`;
  }

  statusText += `\n **Pixabay ( ):**\n`;
  if (config.PIXABAY_KEY) {
    statusText += `      (${config.PIXABAY_KEY.substring(0,4)}...)\n`;
  } else {
    statusText += `    Pixabay   .\n`;
  }
  ...
  const keyboard = {
    inline_keyboard: [[
        { text: "    ", callback_data: "admin_refresh_keys" }
      ],[
        { text: " ", callback_data: "admin_close" }
      ]
    ]
  };
...
async function handleVoiceMessage(message: Message, env: Env, config: ReturnType<typeof createConfig>) {
  ...
  if (!canProcessConcurrentRequest(chat.id, requestId)) {
    await sendMessage(chat.id, "     .      .", { reply_to_message_id: message.message_id });
    return;
  }
  ...
  try {
    ...
    try {
      ...
      const errMsg = lang === 'fa' ? '       .' : ' Could not fetch voice file.';
    ...
    try {
      ...
      let userMessage = lang === 'fa' ? ' **   !**\n\n' : ' **Could not understand!**\n\n';
      
      if (errorMsg.includes('timeout') || errorMsg.includes('')) {
        userMessage += lang === 'fa' ? '>      .' : '>  Processing timed out.';
      } else {
        userMessage += lang === 'fa' ? '>           .' : '>  Please speak clearly or re-record.';
      }
    ...
    if (transcribedText.length < 2) {
      const errMsg = lang === 'fa' ? '    .    .' : ' No speech detected. Please speak clearly.';
      if (loadingMsg) await editMessageText(chat.id, loadingMsg.message_id, errMsg).catch(() => {});
      return;
    }
...
async function handleMediaMessage(message: Message, env: Env, config: any) {
  ...
  if (photo && photo.length > 0) {
    ...
    if (!animation.thumbnail) {
      await sendMessage(chat.id, "   thumbnail     .", { reply_to_message_id: message.message_id });
      return;
    }
  } else if (video) {
    mediaCategory = 'video';
    ...
    if (video.file_size && video.file_size > 8 * 1024 * 1024) {
      await sendMessage(chat.id, "     ( 8 )", { reply_to_message_id: message.message_id });
      return;
    }
  } else if (document) {
  ...
  try {
    ...
    if (isGroup && !session.vipStatus) {
      await sendMessage(chat.id, "      VIP .", { reply_to_message_id: message.message_id });
      return;
    }
    ...
    let loadingIcon = mediaCategory === 'image' ? '' : mediaCategory === 'pdf' ? '' : '';
    let loadingText = lang === 'fa' ? `> ${loadingIcon}   ...` : `> ${loadingIcon} Analyzing...`;
    ...
    if (mediaCategory === 'text_file') {
      ...
      const promptText = lang === 'fa'
        ? `  \`${fileName}\`  .${userQuestion ? ` : ${userQuestion}` : '  .'}\n\n\`\`\`${ext}\n${fileContent}\n\`\`\``
    ...
    if (mediaCategory === 'pdf') {
      ...
      const userQuestion = caption?.trim() || '';
      const promptText = userQuestion || (lang === 'fa' ? '   PDF .' : 'Summarize this PDF.');
...
async function _processAIRequestInternal(
  ...
  if (!isGroup) {
    ...
    const loadingText = lang === 'fa' ? `${emoji} ...` : `${emoji} Hmmm...`;
...
async function sendStreamingResponse(
  ...
  if (!msgId) {
    ...
    const initMsg = await sendMessage(chatId, `${emoji}  ...`, {
...
async function handleGeminiRequest(
  ...
  for (let attempt = 0; attempt < totalKeys; attempt++) {
    ...
    try {
      ...
      if (errorMsg.includes('quota') || errorMsg.includes('429') || errorMsg.includes('rate limit')) {
        ...
        errors.blocked++;
        logger.warn(` Safety block on Key ${keyIndex + 1}`);
        continue;
      } 
      else if (errorMsg.includes('timeout')) {
        errors.timeout++;
        logger.warn(` Timeout on Key ${keyIndex + 1}`);
      } 
      else {
        errors.other++;
  ...
  throw new Error(`   :  ${totalKeys}   .  /model .`);
}
...
async function handleSambanovaRequest(
  ...
  if (config.SAMBANOVA_KEYS.length === 0) throw new Error("  API   ");
  if (config.SAMBANOVA_MODELS.length === 0) {
    const cache = await getModelsWithCache("sambanova", env, true);
    if (cache.models.length === 0) throw new Error("      ");
  }
...
function getStartKeyboard(isGroup: boolean, lang: 'fa' | 'en') {
  ...
  return {
    inline_keyboard: [
      [
        createInlineButton(lang === 'fa' ? '  ' : ' Select Model', 'model_settings'),
        createInlineButton(lang === 'fa' ? ' '      : ' Help',          'open_help'     ),
      ],
      [
        createInlineButton(lang === 'fa' ? '  ' : ' Change Personality', 'personality_menu')
      ]
    ]
  };
}
...
async function handleModelSwitch(session: ChatSession, engine: AIEngine, cb: CallbackQuery, env: Env): Promise<void> {
  ...
  if (!engineInfo.available()) {
    await answerCallbackQuery(cb.id, ` ${engName}   `, true);
    return;
  }

  if (session.activeEngine === engine) {
    await answerCallbackQuery(cb.id, ` ${engName}    `, false);
    return;
  }
  ...
  await answerCallbackQuery(cb.id, `   ${getEngineName(engine, session.language)}`, false);
...
async function handleGroupModeSwitch(session: ChatSession, mode: string, cb: CallbackQuery, env: Env): Promise<void> {
  const modes = {
    'always': { mode: 'always' as const, label: '  ' },
    'mention': { mode: 'mention_only' as const, label: ' ' },
    'smart': { mode: 'smart' as const, label: '' }
  };
  
  const modeInfo = modes[mode];
  session.settings.groupResponseMode = modeInfo.mode;
  await saveSessionWithLock(session, env, true);
  await answerCallbackQuery(cb.id, ` : ${modeInfo.label}`, false);
...
async function handleExistingCallbacks(cb: CallbackQuery, session: ChatSession, env: Env, config: ReturnType<typeof createConfig>) {
  ...
  switch (data) {
    ...
    case 'reset_prompt_pollinations': {
      ...
      await saveSessionWithLock(session, env);
      await answerCallbackQuery(cb.id, ` ${getEngineName('pollinations', session.language)}  `, false);
      await updatePromptMenu(chat.id, cb.message!.message_id, session);
      break;
    }
    case 'show_prompts': {
    ...
    case 'set_personality_victoria':
    {
      ...
      await answerCallbackQuery(cb.id, `  ${emoji} ${name}  !`, false);
      ...
      await editMessageText(chat.id, cb.message.message_id, text, {
        reply_markup: JSON.stringify({
          inline_keyboard: [
            [{ text: lang === 'fa' ? '   ' : ' Back to Personalities', callback_data: 'personality_menu' }],
            [{ text: lang === 'fa' ? '  ' : ' Main Menu', callback_data: 'open_help' }]
          ]
        })
      });
      break;
...
async function updateGroupSettings(chatId: number, messageId: number, session: ChatSession) {
  ...
  const keyboard = {
    inline_keyboard: [
      [ { text: lang === 'fa' ? ' ' : ' Back', callback_data: 'open_help' } ]
    ]
  };
...
async function showHelpChat(cb: CallbackQuery, env: Env): Promise<void> {
  ...
  await editMessageText(cb.message!.chat.id, cb.message!.message_id, text, {
    reply_markup: JSON.stringify({
      inline_keyboard: [[
        { text: lang === 'fa' ? ' ' : ' Back', callback_data: 'help_back' }
      ]]
    })
  });
}
...
async function handleImageGenerationCommand(message: Message, args: string[], env: Env): Promise<void> {
  ...
  if (config.CLOUDFLARE_PAIRS.length === 0) {
    await sendMessage(chat.id, "   Cloudflare AI   .", { reply_to_message_id: message.message_id });
    return;
  }
  ...
  for (let i = 0; i < config.AI_IMAGE_MODELS.length; i++) {
    ...
    try {
      ...
      if (from.id === config.BOT_OWNER_ID) {
        ...
        if (errorMsg.includes('Timeout')) errorMsg = " ";
        else if (errorMsg.includes('NSFW') || errorMsg.includes('safety')) errorMsg = "  ";
        else if (errorMsg.includes('500') || errorMsg.includes('502')) errorMsg = "  ";
        else if (errorMsg.includes('400')) errorMsg = "  ";
        else errorMsg = " ";
        errors.push(`� **${getShortModelName(model)}**: ${errorMsg}`);
      }
    }
...
async function handleAgentCommand(message: Message, args: string[], env: Env) {
  ...
  const toolList = Object.entries(AGENT_TOOLS).map(([n, t]) => 
    `- ${n}: ${t.desc} (: ${JSON.stringify(t.params)})`
  ).join('\n');
  
  const agentSystemPrompt = `          . :\n${toolList}\n\n       JSON  :\n{"tool": "generate_image", "args": {"prompt": " "}}\n      .`;
  ...
  if (toolCall?.tool && AGENT_TOOLS[toolCall.tool]) {
    await editMessageText(chat.id, thinkingMsg.message_id, `   : ${toolCall.tool}...`);
    const result = await executeToolFromAgent(toolCall.tool, toolCall.args, chat.id, message.message_id, env);
    await editMessageText(chat.id, thinkingMsg.message_id, `   .\n${result}\n\n  :`);
  } else {
...
async function handleSearchCommand(message: Message, args: string[], env: Env) {
  ...
  try {
    ...
    await sendImageResults(chat.id, message.message_id, images, query, {
      search_results: txt.search_results || ' {caption}\n\n {count}   ',
    ...
    if (from.id === config.BOT_OWNER_ID) {
      ...
      if (errorMsg === "NO_RESULTS") {
        finalError = "   .     .";
      } else if (errorMsg.includes('quota') || errorMsg.includes('')) {
        finalError = "  .     .";
      } else {
...
async function handleSetPromptCommand(message: Message, args: string[], env: Env) {
  ...
  const engineMap: { [key: string]: AIEngine | undefined } = {
    '': 'gemini', 'nova': 'gemini', 'gemini': 'gemini',
    '': 'sambanova', 'luna': 'sambanova', 'sambanova': 'sambanova',
    '': 'pollinations', 'zara': 'pollinations', 'pollinations': 'pollinations'
  };
  ...
  if (!promptText || promptText.length === 0) {
    await sendMessage(chat.id, lang === 'fa' ? "    " : " Prompt cannot be empty", {
      reply_to_message_id: message.message_id
    });
    return;
  }
...
async function updateAdminPanel(chatId: number, messageId: number, env: Env) {
  ...
  text += ` VIP: ${vipUsers} |  : ${allUsers.length - vipUsers}\n`;
  text += `  : ${activeToday}\n`;
  text += `  : ${blockedCount}\n`;
  text += `  : ${totalMessages}\n\n`;
  text += `  ${state.page + 1}  ${totalPages}\n`;
  text += ` : ${state.sortBy === 'new' ? ' ' : state.sortBy === 'active' ? ' ' : ' '}\n\n`;
  text += `\n\n`;
  ...
  pageUsers.forEach((user, idx) => {
    ...
    text += ` \`${user.userId}\`\n`;
    text += ` ${user.statistics.totalMessages}  \\|  ${lastSeen}\n`;
    text += ` : ${user.dailyLimits.messages}/50 \n\n`;
  });
  ...
  const navRow: any[] = [];
  if (state.page > 0) navRow.push({ text: " ", callback_data: "admin_page_prev" });
  navRow.push({ text: `${state.page + 1}/${totalPages}`, callback_data: "admin_noop" });
  if (state.page < totalPages - 1) navRow.push({ text: " ", callback_data: "admin_page_next" });
  keyboard.inline_keyboard.push(navRow);
  
  keyboard.inline_keyboard.push([
    { text: " ", callback_data: "admin_sort_new" },
    { text: " ", callback_data: "admin_sort_active" },
    { text: " ", callback_data: "admin_sort_messages" }
  ]);
  
  keyboard.inline_keyboard.push([
    { text: isInMaintenance ? "   " : "   ", callback_data: "admin_toggle_maintenance" },
  ...
  keyboard.inline_keyboard.push([
    { text: " ", callback_data: "admin_refresh" },
    { text: " ", callback_data: "admin_close" }
  ]);
...
async function handleBroadcastCallback(cb: CallbackQuery, env: Env) {
  ...
  const keyboard = {
    inline_keyboard: [
      [
        { text: "  ", callback_data: "broadcast_all" },
        { text: "  VIP", callback_data: "broadcast_vip" }
      ],
      [
        { text: "  ", callback_data: "broadcast_free" }
      ],
      [
        { text: " ", callback_data: "admin_back_to_main" }
      ]
    ]
  };
...
async function handleBroadcastMessage(message: Message, broadcastState: any, env: Env) {
  ...
  try {
    ...
    await editMessageText(chat.id, processingMsg.message_id,
      ...
      `  : **${targetUsers.length}** \n` +
      `    ** **  \n` +
      `  : **${Math.ceil(targetUsers.length / 20) * 30} **\n\n` +
      `     batch...`,
      {
        reply_markup: JSON.stringify({
          inline_keyboard: [[
            { text: " ", callback_data: "broadcast_status" },
            { text: " ", callback_data: "broadcast_cancel" }
          ]]
        })
      }
    );
    ...
    broadcastStates.delete(chat.id);
    await editMessageText(chat.id, processingMsg.message_id, "    job").catch(() => {});
  }
}
  ...
  try {
    ...
    if (isDone) {
      await editMessageText(job.adminChatId, job.adminMessageId,
        ...
        {
          reply_markup: JSON.stringify({
            inline_keyboard: [[{ text: " ", callback_data: "broadcast_close" }]]
          })
        }
      ).catch(() => {});
      ...
      await editMessageText(job.adminChatId, job.adminMessageId,
        ` **  ...**\n\n` +
        ` : ${job.processedIndex}/${job.totalUsers} (${progressPercent}%)\n` +
        ` : ${job.sent} |  : ${job.failed}\n` +
        `     ...`,
        {
          reply_markup: JSON.stringify({
...
async function showUserDetail(chatId: number, messageId: number, userId: number, env: Env) {
  ...
  const keyboard = {
    inline_keyboard: [
      ...
      [
        { text: "   ", callback_data: `admin_msg_${userId}` }
      ],
      [
        { text: "  ", callback_data: `admin_view_memory_${userId}` }
      ],
      [
        { text: " ", callback_data: "admin_back_to_main" }
      ]
...
async function showUserMemory(chatId: number, messageId: number, userId: number, env: Env): Promise<void> {
  try {
    ...
    text += ` \`${userId}\`\n`;
    text += `   : **${totalSent}**\n`;
    text += `    : **${historyCount}** (: ${config.HISTORY_LIMIT})\n`;
    text += `  : ${getEngineName(userSession.activeEngine, 'fa')}\n\n`;
    
    text += ` **    :**\n`;
    text += `�  : ${userSession.statistics?.geminiMessages || 0}\n`;
    text += `�  : ${userSession.statistics?.sambanovaMessages || 0}\n`;
    text += `�  : ${userSession.statistics?.pollinationsMessages || 0}\n\n`;
    text += `\n\n`;
    ...
    text += `\n\n`;
    text += ` ** :**\n`;
    text += `�  : ${formatSafeDate(userSession.lastSeen, 'short')}\n`;
    text += `�   :\n`;
    text += `  - : ${userSession.engines.gemini.history.length}\n`;
    text += `  - : ${userSession.engines.sambanova.history.length}\n`;
    text += `  - : ${userSession.engines.pollinations.history.length}\n`;
    
    const keyboard = {
      inline_keyboard: [
        [
          createInlineButton('   ', `admin_download_memory_${userId}`)
        ],
        [
          createInlineButton('  ', `admin_confirm_reset_memory_${userId}`)
        ],
        [
          createInlineButton(' ', `admin_user_${userId}`)
        ]
      ]
    };
...
async function resetUserMemory(chatId: number, messageId: number, userId: number, env: Env): Promise<void> {
  try {
    ...
    await editMessageText(chatId, messageId, successText, {
      reply_markup: JSON.stringify({
        inline_keyboard: [[
          { text: session.language === 'fa' ? ' ' : ' Back', callback_data: `admin_user_${userId}` }
        ]]
      })
    });
    
    logger.info(` Memory reset for user ${userId} by admin`);
...
async function deleteUserSession(chatId: number, messageId: number, userId: number, env: Env): Promise<void> {
  try {
    ...
    logger.info(` Deleted session for user ${userId} by admin`);
  } catch (error) {
    logger.error(`Failed to delete session for ${userId}`, error);
    await sendMessage(chatId, `    : ${error instanceof Error ? error.message : ''}`, {
      reply_to_message_id: messageId
    });
  }
}
...
async function handleRebuildDatabaseCommand(message: Message, env: Env): Promise<void> {
  ...
  try {
      ...
      try {
        ...
        if (!session.userMemories || 
          ...
          if (userId) {
            ...
            logger.warn(` Could not find userId for session ${session.id}`);
            skippedSessions++;
          }
        }
    ...
    let resultText = ` **   !**\n\n`;
    resultText += ` **:**\n`;
    resultText += `�  : ${totalSessions}\n`;
    resultText += `�  : ${fixedSessions}\n`;
    resultText += `�   : ${createdUsers}\n`;
    resultText += `�  : ${skippedSessions}\n\n`;
    
    if (fixedSessions > 0 || createdUsers > 0) {
      resultText += ` **:** ${fixedSessions + createdUsers}   !\n\n`;
      resultText += `   /admin       .`;
    } else {
      resultText += `       .`;
    }
...
async function sendDatabaseStats(chatId: number, replyTo: number, env: Env): Promise<void> {
  const processingMsg = await sendMessage(chatId, "   ...", {
  ...
  try {
    ...
    const text = ` ** **\n\n` +
      ...
      `  : ${totalMessages}\n` +
      `  : ${totalVoices}\n\n` +
      ` : ${formatSafeDate(oldestSession, 'short')}\n` +
      ` : ${formatSafeDate(newestSession, 'short')}`;

    const keyboard = {
      inline_keyboard: [
        [{ text: " ", callback_data: "db_auto_clean" }],
        [{ text: "  ", callback_data: "db_delete_old" }],
        [{ text: " ", callback_data: "db_refresh_stats" }],
        [{ text: " ", callback_data: "admin_back_to_main" }]
      ]
    };
    ...
    await editMessageText(chatId, processingMsg.message_id, "    ");
  }
}
...
async function handleBlockedUsersCommand(message: Message, env: Env): Promise<void> {
  ...
  try {
    ...
    await editMessageText(chat.id, processingMsg.message_id, 
      ` **   ${allUserIds.length} ...**\n\n` +
      `    ( ${Math.ceil(allUserIds.length * 0.15)} )`
    );
    ...
    let text = ` **  **\n\n`;
    text += `  ${allUserIds.length}   :\n`;
    text += ` : ${allUserIds.length - blockedUsers.length}\n`;
    ...
    const keyboard = {
      inline_keyboard: [
        [
          { text: "   ", callback_data: "admin_delete_blocked" }
        ],
        [
          { text: "  ", callback_data: "admin_export_blocked" }
        ],
        [
          { text: " ", callback_data: "admin_back_to_main" }
        ]
...
async function handleSetVipCommand(message: Message, args: string[], env: Env) {
  ...
  if (args.length > 0 && !isNaN(parseInt(args[0]))) {
    ...
    if (!stored) {
      await sendMessage(chat.id, `  \`${targetUserId}\`  .`, {
        reply_to_message_id: message.message_id
      });
      return;
    }
...
async function handleLogCommand(message: Message, env: Env): Promise<void> {
  ...
  const keyboard = {
    inline_keyboard: [
      [
        { text: " ", callback_data: "log_errors" },
        { text: " ", callback_data: "log_warnings" }
      ],
      [
        { text: " ", callback_data: "log_clear" },
        { text: " ", callback_data: "log_refresh" }
      ],
      [
        { text: "  ", callback_data: "log_download" }
      ],
      [
        { text: " ", callback_data: "admin_close" }
      ]
...
async function handleBanCommand(message: Message, args: string[], env: Env): Promise<void> {
  ...
  const formatDuration = (secs: number): string => {
    ...
    if (secs < 3600) return `${Math.floor(secs / 60)} `;
    if (secs < 86400) return `${Math.floor(secs / 3600)} `;
  ...
  try {
    ...
    const banMsg = await sendMessage(chat.id, 
      ` **${banTarget.first_name}**  !\n : **${formatDuration(validSeconds)}**`
    );
...
async function handleDbCleanCommand(message: Message, env: Env): Promise<void> {
  ...
  const cleanMsg = await sendMessage(chat.id, "    ...", {
    reply_to_message_id: message.message_id
  });
...
async function ttsHandler(text: string, chatId: number, replyToMsgId: number, env: Env): Promise<boolean> {
  try {
    const match = text.match(/^([\w\u0600-\u06FF]+)\s+\s+\s+\s+(.+)$/i);
    ...
    if (!ttsText || ttsText.length < 2) {
      await sendMessage(chatId, "    .", { reply_to_message_id: replyToMsgId });
      return true;
    }
  ...
  const keyboard = {
    inline_keyboard: [
      ...
      [
        ...
        { text: " Main Admin Panel", callback_data: "open_admin" }
      ]
    ]
  };
    ...
    await Promise.all(
      engines.map(async engine => {
        try {
          ...
          logger.warn(` Failed to preload ${engine} models`);
        }
      })
    );
...
async function handleCommand(command: string, args: string[], message: Message, env: Env) {
  const { chat, from } = message;
  if (!from) return;
  ...
  switch (command) {
    ...
    case 'search':
      await handleSearchCommand(message, args, env);
      break;
    ...
    case 'resetfactory':
      ...
      const confirmKeyboard = {
        inline_keyboard: [
          [
            { text: "      ", callback_data: "resetfactory_confirm" },
            { text: " ", callback_data: "resetfactory_cancel" }
          ]
        ]
      };
      await sendMessage(chat.id, 
    ...
    case 'agent':
      await handleAgentCommand(message, args, env);
      break;
    ...
    case 'setvip':
      await handleSetVipCommand(message, args, env);
      break;
    ...
    case 'del':
      await handleDeleteCommand(message, env);
      break;

    case 'remove':
      await handleRemoveCommand(message, env);
      break;

    case 'ban':
      await handleBanCommand(message, args, env);
      break;

    case 'dbclean':
      await handleDbCleanCommand(message, env);
      break;

    case 'dbstats':
      await handleDbStatsCommand(message, env);
      break;

    case 'dbdelete':
      await handleDbDeleteCommand(message, args, env);
      break;
    ...
    case 'blocked':
      await handleBlockedCommand(message, env);
      break;

    default:
      await handleUnknownCommand(message, env);
      break;
  }
}
...
async function processGroupMessage(message: Message, env: Env, config: ReturnType<typeof createConfig>) {
  const { chat, from, text } = message;
  if (!text || !from) return;
  ...
  //        
  if (!shouldRespondInGroup(message, session)) {
    return;
  }
...
async function handlePrivateTextMessage(message: Message, env: Env, config: ReturnType<typeof createConfig>) {
  const { chat, from, text } = message;
  if (!text || !from) return;
...
async function handleUnknownCommand(message: Message, env: Env): Promise<void> {
  const { chat } = message;
...
async function handleTextMessage(message: Message, env: Env, config: ReturnType<typeof createConfig>) {
  ...
  // ===== TTS Handler (  ) =====
  const ttsMatch = text.match(/^([\w\u0600-\u06FF]+)\s+\s+\s+\s+(.+)$/i);
  if (ttsMatch) {
  ...
  if (broadcastState && from.id === config.BOT_OWNER_ID) {
    ...
    await handleBroadcastMessage(message, broadcastState, env);
    return;
  }
  ...
  if (text.startsWith('/')) {
    ...
    if (isGroup && text.startsWith('/') && !shouldRespondInGroup(message, await getOrCreateSession(chat, from, env))) {
      return;
    }

    await handleCommand(command, args, message, env);
    return;
  }

  // ===== Group Message Processing =====
  if (isGroup) {
    await processGroupMessage(message, env, config);
    return;
  }

  // ===== Private Message Processing =====
  await handlePrivateTextMessage(message, env, config);
}
...
async function showBotStatusInGroup(message: Message, env: Env): Promise<void> {
  const { chat } = message;
  
  try {
    const botMember = await callTelegramAPI("getChatMember", {
      chat_id: chat.id,
      user_id: (await callTelegramAPI("getMe", {})).id
    });
    
    let statusText = ` **   **\n\n`;
    statusText += `� : ${botMember.status === 'administrator' ? ' ' : '  '}\n`;
    
    if (botMember.status === 'administrator') {
      statusText += `�  :\n`;
      statusText += `  -  : ${botMember.can_delete_messages ? '' : ''}\n`;
      statusText += `  -  : ${botMember.can_restrict_members ? '' : ''}\n`;
      statusText += `  -  : ${botMember.can_invite_users ? '' : ''}\n`;
      statusText += `  -  : ${botMember.can_change_info ? '' : ''}\n`;
    }
    
    await sendMessage(chat.id, statusText, {
      reply_to_message_id: message.message_id
    });
...
console.log(` Total lines: ~${__lineCount || '12000'}`);
console.log(` Owner: @Hamid_Ai_pro`);
console.log(` Created by: Hsoofi82`);
console.log(` Languages: Persian (fa) & English (en)`);
console.log(` AI Engines: Gemini, SambaNova, Pollinations`);
console.log(` Personalities: 10 unique characters`);
console.log(` Features: Text, Voice, Image, PDF, Web Apps`);
