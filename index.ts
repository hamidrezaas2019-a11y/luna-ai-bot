...
const adminPanelStates = new Map<number, AdminPanelState>(); // ✅ پنل ادمین
const modelListStates = new Map<string, ModelListState>(); // ✅ لیست مدل‌ها
const broadcastStates = new Map<number, { mode: 'all' | 'vip' | 'free' | 'specific'; userId?: number }>(); // ✅ برادکست
...
function analyzeGroupConversation(context: any[], currentUser: any, lang: 'fa' | 'en' | 'ar' = 'fa'): string {
  if (!context || context.length === 0) return '';
  
  const recent = context.slice(-3);
  let analysis = '';
  
  const topics = recent.flatMap(msg => {
    const words = msg.text?.toLowerCase().split(/\s+/) || [];
    return words.filter(w => w.length > 3).slice(0, 3);
  });
  
  const common = topics.find((t, i) => topics.indexOf(t) !== i);
  if (common) {
    analysis += lang === 'fa' ? `گروه درباره ${common} صحبت می‌کنه. ` : 
                lang === 'en' ? `Group is discussing ${common}. ` : 
                `المجموعة تناقش ${common}. `;
  }
...
function getGroupContext(chatId: number): any[] {
  const cached = groupContextCache.get(chatId);
  if (!cached) return [];
  
  const now = Date.now();
  const valid = cached.messages
...
// ============================================================
// 📦 TYPES & INTERFACES - بخش پنل ادمین
// ============================================================
...
interface AdminAction {
  ...
  adminId: number;
  adminName: string;
  action: 'ban' | 'unban' | 'warn' | 'mute' | 'unmute' | 'delete' | 'pin' | 'unpin';
  targetUserId?: number;
...
interface AdminReport {
  ...
  data: {
    ...
    vipUsers: number;
    totalMessages: number;
    messagesByEngine: Record<AIEngine, number>;
    activeUsers: number;
    newUsers: number;
    groupsJoined: number;
    groupsLeft: number;
...
interface AdminErrorLog {
  id: string;
  timestamp: number;
  type: 'api' | 'database' | 'system' | 'security';
...
interface AdminSettings {
  ...
  dashboard: {
    ...
    defaultView: 'summary' | 'details' | 'charts';
...
// ════════════════════════════════════════════════════════════
// 👑 ADMIN PANEL V2.0 — ماژول کامل و خودکفا
// ════════════════════════════════════════════════════════════
...
interface AdminPanelState {
  ...
  perPage: number;
  sortBy: 'new' | 'active' | 'messages';
  currentSection: 'dashboard' | 'users' | 'stats' | 'logs' | 'settings' | 'broadcast';
  searchQuery?: string;
}

// ============================================================
// 📌 IMPORTANT: اگر adminPanelStates قبلاً تعریف شده، این خط رو حذف کن
// ============================================================
// const adminPanelStates = new Map<number, AdminPanelState>();
// ============================================================

function getAdminState(chatId: number): AdminPanelState {
  return adminPanelStates.get(chatId) || {
    page: 0, perPage: 5, sortBy: 'new', currentSection: 'dashboard'
  };
}
...
function createProgressBar(percent: number, length: number = 12): string {
  ...
  try {
    ...
    for (const item of allKeys) {
      try {
        ...
        if (session.statistics.totalMessages < 1 && !session.vipStatus) continue;
        ...
        users.push({
          ...
          statistics: {
            ...
            geminiMessages: session.statistics.geminiMessages || 0,
            sambanovaMessages: session.statistics.sambanovaMessages || 0,
            pollinationsMessages: session.statistics.pollinationsMessages || 0,
            voicesReceived: session.statistics.voicesReceived || 0,
            firstUsed: session.statistics.firstUsed || session.lastSeen,
            lastSeen: session.lastSeen || Date.now()
          },
          activeEngine: session.activeEngine || 'gemini',
          vipStatus: session.vipStatus || false,
          dailyLimits: session.dailyLimits || { messages: 0, voicesSent: 0, voicesReceived: 0, imagesGenerated: 0 }
        });
      } catch (error) { continue; }
    }
  } catch (error) {
    logger.error("Failed to get all user statistics", error);
  }
  return users;
}

async function getBlockedUsersList(env: Env): Promise<Array<{ userId: number; since: number; reason: string }>> {
  const blockedUsers: Array<{ userId: number; since: number; reason: string }> = [];
  try {
    let allKeys: any[] = [];
    let listResult = await env.SESSIONS.list({ prefix: "user_blocked:" });
    allKeys.push(...listResult.keys);
    while (!listResult.list_complete && listResult.cursor) {
      listResult = await env.SESSIONS.list({ prefix: "user_blocked:", cursor: listResult.cursor });
      allKeys.push(...listResult.keys);
    }
    for (const item of allKeys) {
      try {
        const stored = await env.SESSIONS.get(item.name, "json");
        if (!stored) continue;
        blockedUsers.push(stored as { userId: number; since: number; reason: string });
      } catch (error) {}
    }
  } catch (error) {
...
async function isUserBlockedAdmin(userId: number, env: Env): Promise<boolean> {
  try {
    const stored = await env.SESSIONS.get(`user_blocked:${userId}`, "json");
    return !!stored;
  } catch (error) { return false; }
}

async function setUserBlockedAdmin(userId: number, blocked: boolean, env: Env, reason: string = "By admin"): Promise<void> {
  try {
    const key = `user_blocked:${userId}`;
    if (blocked) {
      await env.SESSIONS.put(key, JSON.stringify({ userId, since: Date.now(), reason }));
    } else {
      await env.SESSIONS.delete(key);
    }
    logger.info(`User ${userId} ${blocked ? 'blocked' : 'unblocked'}`);
  } catch (error) {
    logger.error(`Failed to set blocked status for ${userId}`, error);
  }
}

async function countKeysAdmin(prefix: string, env: Env): Promise<number> {
  try {
    let count = 0;
...
async function cleanupSessionsAdmin(env: Env): Promise<{ deleted: number; errors: number }> {
  ...
  try {
    ...
    for (const item of allKeys) {
      try {
        ...
        if (!stored) continue;
        const session = stored as ChatSession;
        if (session.lastSeen < thirtyDaysAgo && session.messageCount < 5) {
          await env.SESSIONS.delete(item.name);
          deleted++;
        }
      } catch (error) { errors++; }
    }
    logger.info(`🧹 Cleanup: ${deleted} deleted, ${errors} errors`);
  } catch (error) {
...
async function exportCSVAdmin(chatId: number, env: Env): Promise<void> {
  const users = await getAllUserStatistics(env);
  let csv = "User ID,First Name,Username,VIP Status,Total Messages,Gemini,SambaNova,Pollinations,Voices,Daily Messages,Daily Voices,Daily Images,First Used,Last Seen\n";
  users.forEach(u => {
    const iso = (t?: number) => { if (!t || isNaN(t)) return 'N/A'; try { return new Date(t).toISOString(); } catch { return 'N/A'; } };
    csv += `${u.userId},"${u.firstName.replace(/"/g, '""')}","${(u.userName || 'N/A').replace(/"/g, '""')}",`;
    csv += `${u.vipStatus ? 'VIP' : 'Free'},${u.statistics.totalMessages || 0},${u.statistics.geminiMessages || 0},`;
    csv += `${u.statistics.sambanovaMessages || 0},${u.statistics.pollinationsMessages || 0},${u.statistics.voicesReceived || 0},`;
    csv += `${u.dailyLimits.messages || 0},${u.dailyLimits.voicesSent || 0},${u.dailyLimits.imagesGenerated || 0},`;
    csv += `"${iso(u.statistics.firstUsed)}","${iso(u.statistics.lastSeen)}"\n`;
  });
  const blob = new Blob([csv], { type: "text/csv; charset=utf-8" });
  const formData = new FormData();
  formData.append("chat_id", chatId.toString());
  formData.append("document", blob, `nova_stats_${Date.now()}.csv`);
  formData.append("caption", "📊 **گزارش کامل کاربران**");
  await fetchWithTimeout(`${API_URL}/sendDocument`, { method: "POST", body: formData });
}

async function showUserMemoryAdmin(chatId: number, messageId: number, userId: number, env: Env): Promise<void> {
  try {
    const stored = await env.SESSIONS.get(`session:${userId}`, "json");
    if (!stored) { await editMessageText(chatId, messageId, "❌ **سشن یافت نشد**"); return; }

    const userSession = stored as ChatSession;
    const allUsers = await getAllUserStatistics(env);
    const userName = allUsers.find(u => u.userId === userId)?.firstName || 'Unknown';
    const engine = userSession.engines[userSession.activeEngine];
    const history = engine.history || [];

    let text = `🧠 **حافظه کاربر ${userName}**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
    text += `🆔 \`${userId}\`\n🤖 موتور: ${getEngineName(userSession.activeEngine, 'fa')}\n💾 پیام‌ها: ${history.length}\n\n`;
    ...
    if (history.length === 0) {
      ...
      history.slice(-10).forEach((item) => {
        const role = item.role === 'user' ? '👤' : item.role === 'model' ? '🤖' : '⚙️';
        const ts = item.timestamp ? new Date(item.timestamp).toLocaleString('fa-IR', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : '?';
        const msg = item.parts[0]?.text || '[رسانه]';
    ...
    const keyboard = {
      inline_keyboard: [
        [{ text: "🔴 ریست 🗑️", callback_data: `admin_confirm_reset_memory_${userId}` }],
        [{ text: "🔙 بازگشت", callback_data: `admin_user_${userId}` }]
      ]
    };
    await editMessageText(chatId, messageId, text, { reply_markup: JSON.stringify(validateKeyboard(keyboard)) });
  } catch (error) {
    logger.error(`Failed to show memory for ${userId}`, error);
    await editMessageText(chatId, messageId, "❌ **خطا در نمایش حافظه**");
  }
}

async function resetUserMemoryAdmin(chatId: number, messageId: number, userId: number, env: Env): Promise<void> {
  try {
    const stored = await env.SESSIONS.get(`session:${userId}`, "json");
    if (!stored) { await editMessageText(chatId, messageId, "❌ **سشن یافت نشد**"); return; }
    const session = stored as ChatSession;
    ...
    await env.SESSIONS.put(`session:${userId}`, JSON.stringify(session));
    await editMessageText(chatId, messageId, `✅ **حافظه کاربر ${userId} ریست شد.**`);
  } catch (error) {
...
async function handleKeysCommandAdmin(chatId: number, messageId: number | undefined, env: Env, isEdit = false): Promise<void> {
  let currentMsgId = messageId;
  const loadingText = `🔍 **در حال بررسی کلیدها...**`;
  ...
  let text = `🔑 **وضعیت کلیدهای API**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
  text += `🤖 **Gemini - ${config.GEMINI_KEYS.length} کلید**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
  for (let i = 0; i < config.GEMINI_KEYS.length; i++) {
    const key = config.GEMINI_KEYS[i];
    const masked = key.substring(0, 6) + '...' + key.substring(key.length - 4);
    const disabled = await isKeyDisabled(key, env);
    text += `${i + 1}. \`${masked}\` ${disabled ? '🔴 غیرفعال' : '🟢 فعال'}\n`;
  }
  ...
  text += `\n🔬 **Pollinations**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
  text += `🔑 ${config.POLLINATIONS_KEY ? '🟢 کلید تنظیم شده' : '🟡 حالت عمومی'}\n`;
  text += `\n⏰ ${new Date().toLocaleTimeString('fa-IR')}`;

  const keyboard = {
    inline_keyboard: [
      [{ text: "🔄 بروزرسانی", callback_data: "admin_refresh_keys" }],
      [{ text: "📊 بازگشت", callback_data: "admin_section_dashboard" }]
    ]
  };
  await editMessageText(chatId, currentMsgId!, text, { reply_markup: JSON.stringify(validateKeyboard(keyboard)) });
}
...
async function showLogsAdmin(chatId: number, msgId: number, env: Env): Promise<void> {
  const logs = (globalThis as any).recentLogs || [];
  if (logs.length === 0) {
    await editMessageText(chatId, msgId, "📭 **هیچ لاگی ثبت نشده است**");
    return;
  }
  let text = `📋 **لاگ‌های سیستم**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
  ...
  const keyboard = {
    inline_keyboard: [
      [{ text: "🗑️ پاک کردن", callback_data: "log_clear" }, { text: "📥 دانلود", callback_data: "log_download" }],
      [{ text: "📊 بازگشت", callback_data: "admin_section_dashboard" }]
    ]
  };
  await editMessageText(chatId, msgId, text, { reply_markup: JSON.stringify(validateKeyboard(keyboard)) });
}
...
async function renderAdminDashboard(chatId: number, messageId: number, env: Env): Promise<void> {
  ...
  const totalMessages = allUsers.reduce((s, u) => s + u.statistics.totalMessages, 0);
  const totalVoices = allUsers.reduce((s, u) => s + (u.statistics.voicesReceived || 0), 0);

  const now = Date.now();
  const activeToday = allUsers.filter(u => now - u.statistics.lastSeen < 86400000).length;
  const activeWeek = allUsers.filter(u => now - u.statistics.lastSeen < 604800000).length;
  const blockedCount = (await getBlockedUsersList(env)).length;

  const yesterday = now - 86400000;
  const usersYesterday = allUsers.filter(u => u.statistics.firstUsed < yesterday).length;
  const growth = totalUsers - usersYesterday;
  const growthIcon = growth > 0 ? '📈' : growth < 0 ? '📉' : '➖';

  const geminiMsgs = allUsers.reduce((s, u) => s + (u.statistics.geminiMessages || 0), 0);
  const sambanovaMsgs = allUsers.reduce((s, u) => s + (u.statistics.sambanovaMessages || 0), 0);
  const pollinationsMsgs = allUsers.reduce((s, u) => s + (u.statistics.pollinationsMessages || 0), 0);
  const totalMsgs = geminiMsgs + sambanovaMsgs + pollinationsMsgs || 1;

  const geminiPct = Math.round((geminiMsgs / totalMsgs) * 100);
  const sambaPct = Math.round((sambanovaMsgs / totalMsgs) * 100);
  const pollPct = Math.round((pollinationsMsgs / totalMsgs) * 100);

  const isInMaintenance = (await env.SESSIONS.get("maintenance_mode", "text")) === "true";

  let text = `👑 **داشبورد مدیریت**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
  text += `📊 **آمار کلی**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
  text += `👥 کل: **${totalUsers}** ${growthIcon} ${growth > 0 ? '+' : ''}${growth}\n`;
  text += `👑 VIP: **${vipUsers}** ｜ 🆓 رایگان: **${freeUsers}**\n`;
  text += `🔥 امروز: **${activeToday}** ｜ هفته: **${activeWeek}**\n`;
  text += `🚫 مسدود: **${blockedCount}**\n`;
  text += `💬 پیام‌ها: **${totalMessages.toLocaleString()}** ｜ 🎤 ویس: **${totalVoices.toLocaleString()}**\n\n`;

  text += `🤖 **مصرف موتورها**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
  text += `🤖 نوا:  [${createProgressBar(geminiPct)}] ${geminiPct}%\n`;
  text += `🧠 لونا: [${createProgressBar(sambaPct)}] ${sambaPct}%\n`;
  text += `✨ زارا: [${createProgressBar(pollPct)}] ${pollPct}%\n\n`;

  text += `⚙️ **وضعیت سیستم**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
  text += `${isInMaintenance ? '🟡 تعمیرات' : '🟢 سالم'}\n`;
  text += `🔑 Gemini: **${config.GEMINI_KEYS.length}** ｜ SambaNova: **${config.SAMBANOVA_KEYS.length}**\n`;
  text += `📦 کش مدل‌ها: **${await countKeysAdmin('model_cache:', env)}**`;

  const keyboard = {
    inline_keyboard: [
      [
        { text: "🟢 جدید ✅", callback_data: "admin_sort_new" },
        { text: "🟢 فعال ⚡", callback_data: "admin_sort_active" },
        { text: "🟢 پرپیام 💬", callback_data: "admin_sort_messages" }
      ],
      [
        { text: "👥 مدیریت کاربران", callback_data: "admin_section_users" }
      ],
      [
        { text: "🔴 ریست همه حافظه‌ها 🗑️", callback_data: "admin_reset_all_memory" },
        { text: "🔴 مشاهده حافظه‌ها 👁️", callback_data: "admin_view_all_memory" }
      ],
      [
        { text: "🟠 ارسال همگانی 📢", callback_data: "admin_broadcast" },
        { text: "🟡 آمار دیتابیس 📊", callback_data: "admin_section_stats" }
      ],
      [
        { text: "🟡 حالت تعمیرات 🔧", callback_data: "admin_toggle_maintenance" },
        { text: "🔵 CSV 📥", callback_data: "admin_export_csv" }
      ],
      [
        { text: "🔵 کاربران مسدود 🚫", callback_data: "admin_show_blocked" },
        { text: "🟢 آنبلاک همه 🔓", callback_data: "admin_unlock_all" }
      ],
      [
        { text: "🟢 آنبلاک کلیدها 🔑", callback_data: "admin_unlock_keys" },
        { text: "🟣 پنل VIP 👑", callback_data: "admin_vip_panel" }
      ],
      [
        { text: "⚪️ وضعیت API Key ها 🔑", callback_data: "admin_show_keys" },
        { text: "⚪️ لاگ‌ها 📋", callback_data: "admin_section_logs" }
      ],
      [
        { text: "⚪️ تنظیمات سیستم ⚙️", callback_data: "admin_section_settings" },
        { text: "⚪️ بروزرسانی 🔄", callback_data: "admin_refresh" }
      ],
      [
...
async function renderAdminUsers(chatId: number, messageId: number, env: Env): Promise<void> {
  const state = getAdminState(chatId);
  const allUsers = await getAllUserStatistics(env);
  ...
  if (state.sortBy === 'new') sortedUsers.sort((a, b) => (b.statistics.firstUsed || 0) - (a.statistics.firstUsed || 0));
  else if (state.sortBy === 'active') sortedUsers.sort((a, b) => (b.statistics.lastSeen || 0) - (a.statistics.lastSeen || 0));
  else sortedUsers.sort((a, b) => (b.statistics.totalMessages || 0) - (a.statistics.totalMessages || 0));

  const totalPages = Math.max(1, Math.ceil(sortedUsers.length / state.perPage));
  const startIdx = state.page * state.perPage;
  const endIdx = Math.min(startIdx + state.perPage, sortedUsers.length);
  const pageUsers = sortedUsers.slice(startIdx, endIdx);

  let text = `👥 **مدیریت کاربران**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
  text += `👥 کل: **${allUsers.length}** ｜ ${state.searchQuery ? `🔍 "${state.searchQuery}"` : 'همه'}\n`;
  text += `📄 صفحه ${state.page + 1}/${totalPages}\n\n`;
  ...
  if (pageUsers.length === 0) {
    ...
    pageUsers.forEach((user, i) => {
      ...
      const badge = user.vipStatus ? '👑 ' : '🆓 ';
      const lastSeen = user.statistics.lastSeen ? new Date(user.statistics.lastSeen).toLocaleDateString('fa-IR', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : '?';
      text += `${num}. ${badge}**${user.firstName}**\n🆔 \`${user.userId}\` ｜ 💬 ${user.statistics.totalMessages || 0}\n📅 ${lastSeen}\n\n`;
    });
  }

  const keyboard: any = { inline_keyboard: [] };
  const userRow: any[] = pageUsers.map((u, idx) => ({ text: `${startIdx + idx + 1}`, callback_data: `admin_user_${u.userId}` }));
  for (let i = 0; i < userRow.length; i += 5) keyboard.inline_keyboard.push(userRow.slice(i, i + 5));

  const navRow: any[] = [];
  if (state.page > 0) navRow.push({ text: '◀️ قبلی', callback_data: 'admin_users_prev' });
  navRow.push({ text: `${state.page + 1}/${totalPages}`, callback_data: 'admin_noop' });
  if (state.page < totalPages - 1) navRow.push({ text: 'بعدی ▶️', callback_data: 'admin_users_next' });
  if (navRow.length) keyboard.inline_keyboard.push(navRow);

  keyboard.inline_keyboard.push([
    { text: "🟢 جدید", callback_data: "admin_sort_new" },
    { text: "🟢 فعال", callback_data: "admin_sort_active" },
    { text: "🟢 پیام", callback_data: "admin_sort_messages" }
  ]);
  keyboard.inline_keyboard.push([
    { text: "🔍 جستجو", callback_data: "admin_search_user" },
    { text: "📊 داشبورد", callback_data: "admin_section_dashboard" }
  ]);
  keyboard.inline_keyboard.push([{ text: "🔴 بستن ❌", callback_data: "admin_close" }]);
...
async function showUserDetailAdmin(chatId: number, messageId: number, userId: number, env: Env): Promise<void> {
  try {
    const stored = await env.SESSIONS.get(`session:${userId}`, "json");
    if (!stored) { await editMessageText(chatId, messageId, "❌ **کاربر یافت نشد**"); return; }

    const allUsers = await getAllUserStatistics(env);
    const u = allUsers.find(x => x.userId === userId);
    if (!u) { await editMessageText(chatId, messageId, "❌ **آمار یافت نشد**"); return; }

    const isBlocked = await isUserBlockedAdmin(userId, env);
    const isVip = u.vipStatus;

    let text = `👤 **${u.firstName}**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
    text += `🆔 \`${u.userId}\`\n👤 @${u.userName || 'ندارد'}\n`;
    text += `👑 ${isVip ? '✅ VIP' : '🆓 رایگان'} ｜ 🔒 ${isBlocked ? '🚫 مسدود' : '✅ فعال'}\n\n`;
    text += `📊 **پیام‌ها:**\n💬 کل: ${u.statistics.totalMessages}\n🤖 نوا: ${u.statistics.geminiMessages}\n`;
    text += `🎨 لونا: ${u.statistics.sambanovaMessages}\n🔬 زارا: ${u.statistics.pollinationsMessages}\n`;
    text += `🎤 ویس: ${u.statistics.voicesReceived}\n\n`;
    text += `📅 اولین: ${formatSafeDate(u.statistics.firstUsed, 'full')}\nآخرین: ${formatSafeDate(u.statistics.lastSeen, 'full')}\n\n`;
    text += `📊 **امروز:** 💬 ${u.dailyLimits.messages}/50 ｜ 🎤 ${u.dailyLimits.voicesSent}/7 ｜ 🖼️ ${u.dailyLimits.imagesGenerated}/5`;

    const keyboard = {
      inline_keyboard: [
        [
          { text: "🔴 ریست حافظه 🗑️", callback_data: `admin_confirm_reset_memory_${userId}` },
          { text: "🔴 مشاهده حافظه 👁️", callback_data: `admin_view_memory_${userId}` }
        ],
        [
          { text: "🟠 پیام خصوصی 📨", callback_data: `admin_msg_${userId}` },
          { text: isBlocked ? "🟢 آنبلاک 🔓" : "🔴 بلاک 🚫", callback_data: `admin_block_${userId}` }
        ],
        [
          { text: isVip ? "🔴 حذف VIP" : "🟣 افزودن VIP 👑", callback_data: `admin_toggle_vip_${userId}` }
        ],
        [{ text: "📊 بازگشت", callback_data: "admin_section_users" }]
      ]
    };
    await editMessageText(chatId, messageId, text, { reply_markup: JSON.stringify(validateKeyboard(keyboard)) });
  } catch (error) {
    logger.error(`Failed to show user detail ${userId}`, error);
    await editMessageText(chatId, messageId, "❌ **خطا**");
  }
}
...
async function renderAdminStats(chatId: number, messageId: number, env: Env): Promise<void> {
  let totalSessions = 0, activeSessions = 0, vipCount = 0, totalMessages = 0, totalVoices = 0;
  ...
  for (const item of allKeys) {
    try {
      const stored = await env.SESSIONS.get(item.name, "json");
      if (!stored) continue;
      const s = stored as ChatSession;
      totalSessions++;
      if (s.lastSeen > sevenDaysAgo) activeSessions++;
      if (s.vipStatus) vipCount++;
      totalMessages += s.statistics?.totalMessages || 0;
      totalVoices += s.statistics?.voicesReceived || 0;
      if (s.statistics?.firstUsed && s.statistics.firstUsed < oldestSession) oldestSession = s.statistics.firstUsed;
      if (s.lastSeen > newestSession) newestSession = s.lastSeen;
    } catch (error) {}
  }

  const modelCacheCount = await countKeysAdmin("model_cache:", env);
  const groupVIPCount = await countKeysAdmin("group_vip:", env);
  const blockedCount = (await getBlockedUsersList(env)).length;

  let text = `📊 **آمار دیتابیس**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
  text += `👥 کل سشن‌ها: **${totalSessions}**\n🔥 فعال (۷ روز): **${activeSessions}**\n`;
  text += `👑 VIP: **${vipCount}** ｜ 🚫 مسدود: **${blockedCount}**\n💤 غیرفعال: **${totalSessions - activeSessions}**\n\n`;
  text += `💬 کل پیام‌ها: **${totalMessages.toLocaleString()}**\n🎤 کل ویس‌ها: **${totalVoices.toLocaleString()}**\n\n`;
  text += `📦 کش مدل‌ها: **${modelCacheCount}** ｜ گروه VIP: **${groupVIPCount}**\n\n`;
  text += `🕰️ قدیمی‌ترین: **${new Date(oldestSession).toLocaleDateString('fa-IR')}**\n`;
  text += `🕰️ جدیدترین: **${new Date(newestSession).toLocaleDateString('fa-IR')}**`;

  const keyboard = {
    inline_keyboard: [
      [
        { text: "🟡 پاکسازی 🛠️", callback_data: "db_auto_clean" },
        { text: "🔴 حذف قدیمی‌ها 🗑️", callback_data: "db_delete_old" }
      ],
      [
        { text: "🔵 CSV 📥", callback_data: "admin_export_csv" },
        { text: "📊 داشبورد", callback_data: "admin_section_dashboard" }
...
async function renderAdminBroadcast(chatId: number, messageId: number, env: Env): Promise<void> {
  const allUsers = await getAllUserStatistics(env);
  const vipCount = allUsers.filter(u => u.vipStatus).length;
  const freeCount = allUsers.length - vipCount;

  let text = `📢 **ارسال همگانی**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
  text += `👥 همه: **${allUsers.length}** نفر\n👑 VIP: **${vipCount}** نفر\n🆓 رایگان: **${freeCount}** نفر`;

  const keyboard = {
    inline_keyboard: [
      [{ text: "👥 همه کاربران", callback_data: "broadcast_all" }, { text: "👑 فقط VIP", callback_data: "broadcast_vip" }],
      [{ text: "🆓 فقط رایگان", callback_data: "broadcast_free" }],
      [{ text: "📊 داشبورد", callback_data: "admin_section_dashboard" }]
    ]
  };
  const state = getAdminState(chatId);
  state.currentSection = 'broadcast';
  adminPanelStates.set(chatId, state);
  await editMessageText(chatId, messageId, text, { reply_markup: JSON.stringify(validateKeyboard(keyboard)) });
}
...
async function renderAdminSettings(chatId: number, messageId: number, env: Env): Promise<void> {
  const isInMaintenance = (await env.SESSIONS.get("maintenance_mode", "text")) === "true";

  let text = `⚙️ **تنظیمات سیستم**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
  text += `🛠️ تعمیرات: ${isInMaintenance ? '🟡 فعال' : '🟢 غیرفعال'}\n`;
  text += `🔑 Gemini: **${config.GEMINI_KEYS.length}** ｜ SambaNova: **${config.SAMBANOVA_KEYS.length}**`;
  ...
  const keyboard = {
    inline_keyboard: [
      [
        ...
        { text: "🔑 وضعیت کلیدها", callback_data: "admin_show_keys" }
      ],
      [
        { text: "🔴 ریست همه حافظه‌ها", callback_data: "admin_reset_all_memory" },
...
// ════════════════════════════════════════════════════════════
// 🎯 MAIN ENTRY POINT #1 — کالبک‌ها
// ════════════════════════════════════════════════════════════

async function handleAdminPanelCallback(cb: CallbackQuery, env: Env): Promise<boolean> {
  if (!cb.message || !cb.data) return false;
  const data = cb.data;
  const chat = cb.message.chat;
  const user = cb.from;

  const isAdminAction =
    data.startsWith('admin_') || data.startsWith('db_') || data.startsWith('log_') ||
    data.startsWith('broadcast_') || data === 'rebuild_database';

  if (!isAdminAction) return false;
  ...
  try {
    ...
    if (data.startsWith('admin_section_')) {
      ...
      const section = data.replace('admin_section_', '');
      await answerCallbackQuery(cb.id);
      switch (section) {
        case 'dashboard': await renderAdminDashboard(chat.id, cb.message.message_id, env); break;
        case 'users': await renderAdminUsers(chat.id, cb.message.message_id, env); break;
        case 'stats': await renderAdminStats(chat.id, cb.message.message_id, env); break;
        case 'settings': await renderAdminSettings(chat.id, cb.message.message_id, env); break;
        case 'broadcast': await renderAdminBroadcast(chat.id, cb.message.message_id, env); break;
        case 'logs': await showLogsAdmin(chat.id, cb.message.message_id, env); break;
        default: await renderAdminDashboard(chat.id, cb.message.message_id, env);
      }
      return true;
    }

    // 👥 ناوبری کاربران
    if (data === 'admin_users_prev' || data === 'admin_users_next') {
      if (!await requireOwner()) return true;
      const state = getAdminState(chat.id);
      const allUsers = await getAllUserStatistics(env);
      const maxPage = Math.max(0, Math.ceil(allUsers.length / state.perPage) - 1);
      state.page = data === 'admin_users_prev' ? Math.max(0, state.page - 1) : Math.min(maxPage, state.page + 1);
      adminPanelStates.set(chat.id, state);
      await answerCallbackQuery(cb.id);
      await renderAdminUsers(chat.id, cb.message.message_id, env);
      return true;
    }
    ...
    if (data === 'admin_search_user') {
      if (!await requireOwner()) return true;
      await answerCallbackQuery(cb.id, "🔍 آیدی یا اسم کاربر را در چت بفرستید", true);
      const state = getAdminState(chat.id);
      state.searchQuery = '';
      adminPanelStates.set(chat.id, state);
      return true;
    ...
    if (data === 'admin_sort_new' || data === 'admin_sort_active' || data === 'admin_sort_messages') {
      ...
      if (state.currentSection === 'users') await renderAdminUsers(chat.id, cb.message.message_id, env);
      else await renderAdminDashboard(chat.id, cb.message.message_id, env);
      return true;
    }
    ...
    if (data.startsWith('admin_user_')) {
      ...
      if (!/^\d+$/.test(rawId)) { await answerCallbackQuery(cb.id, "❌ شناسه نامعتبر", true); return true; }
      await answerCallbackQuery(cb.id);
      await showUserDetailAdmin(chat.id, cb.message.message_id, parseInt(rawId), env);
      return true;
    }
    ...
    if (data.startsWith('admin_toggle_vip_')) {
      if (!await requireOwner()) return true;
      const targetId = parseInt(data.replace('admin_toggle_vip_', ''));
      const stored = await env.SESSIONS.get(`session:${targetId}`, "json");
      if (!stored) { await answerCallbackQuery(cb.id, "❌ کاربر یافت نشد", true); return true; }
      const s = stored as ChatSession;
      s.vipStatus = !s.vipStatus;
      await env.SESSIONS.put(`session:${targetId}`, JSON.stringify(s));
      await answerCallbackQuery(cb.id, s.vipStatus ? "✅ VIP فعال شد" : "❌ VIP حذف شد", false);
      try {
        await sendMessage(targetId, s.vipStatus ? `🎉 **تبریک!**\n\nاکانت شما VIP شد! 👑` : `📢 VIP شما غیرفعال شد.`);
      } catch (e) {}
      await showUserDetailAdmin(chat.id, cb.message.message_id, targetId, env);
      return true;
    }
    ...
    if (data.startsWith('admin_block_')) {
      if (!await requireOwner()) return true;
      const targetId = parseInt(data.replace('admin_block_', ''));
      const isBlocked = await isUserBlockedAdmin(targetId, env);
      if (isBlocked) {
        await setUserBlockedAdmin(targetId, false, env);
        await answerCallbackQuery(cb.id, "✅ آنبلاک شد", false);
        try { await sendMessage(targetId, `✅ حساب شما آزاد شد! 🎉`); } catch (e) {}
      } else {
        await setUserBlockedAdmin(targetId, true, env);
        await answerCallbackQuery(cb.id, "🚫 بلاک شد", false);
        try { await sendMessage(targetId, `🚫 حساب شما مسدود شد.`); } catch (e) {}
      }
      await showUserDetailAdmin(chat.id, cb.message.message_id, targetId, env);
      return true;
    }
    ...
    if (data.startsWith('admin_msg_')) {
      if (!await requireOwner()) return true;
      const targetId = parseInt(data.replace('admin_msg_', ''));
      broadcastStates.set(chat.id, { mode: 'specific', userId: targetId });
      await answerCallbackQuery(cb.id);
    ...
    if (data.startsWith('admin_view_memory_')) {
      if (!await requireOwner()) return true;
      const targetId = parseInt(data.replace('admin_view_memory_', ''));
      await answerCallbackQuery(cb.id, "🧠 در حال بارگذاری...", false);
      await showUserMemoryAdmin(chat.id, cb.message.message_id, targetId, env);
      return true;
    }

    if (data.startsWith('admin_confirm_reset_memory_')) {
      if (!await requireOwner()) return true;
      const targetId = parseInt(data.replace('admin_confirm_reset_memory_', ''));
      await answerCallbackQuery(cb.id);
      await editMessageText(chat.id, cb.message.message_id,
        `⚠️ **تایید ریست حافظه کاربر \`${targetId}\`**\n\nاین عمل غیرقابل بازگشت است!`,
        {
          reply_markup: JSON.stringify({
            inline_keyboard: [[
              { text: "✅ بله، ریست شود", callback_data: `admin_do_reset_memory_${targetId}` },
              { text: "❌ لغو", callback_data: `admin_view_memory_${targetId}` }
            ]]
          })
        });
      return true;
    ...
    if (data.startsWith('admin_do_reset_memory_')) {
      if (!await requireOwner()) return true;
      const targetId = parseInt(data.replace('admin_do_reset_memory_', ''));
      await answerCallbackQuery(cb.id, "🗑️ در حال ریست...", false);
      await resetUserMemoryAdmin(chat.id, cb.message.message_id, targetId, env);
      return true;
    }

    // 🗑️ ریست همه حافظه‌ها
    if (data === 'admin_reset_all_memory') {
      if (!await requireOwner()) return true;
      await answerCallbackQuery(cb.id, "⚠️ در حال ریست همه...", true);
      let allKeys: any[] = [];
      let listResult = await env.SESSIONS.list({ prefix: "session:" });
      allKeys.push(...listResult.keys);
      while (!listResult.list_complete && listResult.cursor) {
      ...
      for (const item of allKeys) {
        try {
          ...
          const engines: AIEngine[] = ['gemini', 'sambanova', 'pollinations'];
          for (const eng of engines) {
            if (s.engines[eng]) { s.engines[eng].history = []; s.engines[eng].userHistories = new Map(); }
          }
          s.messageCount = 0;
          s.statistics.totalMessages = 0;
          await env.SESSIONS.put(item.name, JSON.stringify(s));
          resetCount++;
        } catch (e) {}
      }
      await sendMessage(chat.id, `✅ **${resetCount}** حافظه ریست شد.`);
      return true;
    }

    // 👁️ مشاهده همه حافظه‌ها
    if (data === 'admin_view_all_memory') {
      if (!await requireOwner()) return true;
      await answerCallbackQuery(cb.id, "🧠 در حال بارگذاری...", false);
      const allUsers = await getAllUserStatistics(env);
      const withMemory = allUsers.filter(u => u.statistics.totalMessages > 0);
      let text = `🧠 **کاربران دارای حافظه (${withMemory.length})**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
      withMemory.slice(0, 20).forEach((u, i) => { text += `${i + 1}. **${u.firstName}** ｜ 🆔 \`${u.userId}\` ｜ 💬 ${u.statistics.totalMessages}\n`; });
      if (withMemory.length > 20) text += `\n➕ و ${withMemory.length - 20} کاربر دیگر`;
      await editMessageText(chat.id, cb.message.message_id, text, {
        reply_markup: JSON.stringify({ inline_keyboard: [[{ text: "📊 داشبورد", callback_data: "admin_section_dashboard" }]] })
      });
      return true;
    }

    // 🔓 آنبلاک همه کاربران
    if (data === 'admin_unlock_all') {
      if (!await requireOwner()) return true;
      await answerCallbackQuery(cb.id, "🔓 در حال آنبلاک همه...", false);
      let allKeys: any[] = [];
      let listResult = await env.SESSIONS.list({ prefix: "user_blocked:" });
      allKeys.push(...listResult.keys);
      while (!listResult.list_complete && listResult.cursor) {
      ...
      let count = 0;
      for (const item of allKeys) { try { await env.SESSIONS.delete(item.name); count++; } catch (e) {} }
      await sendMessage(chat.id, `✅ **${count}** کاربر آنبلاک شدند.`);
      return true;
    }

    // 🔓 آنبلاک کلیدها
    if (data === 'admin_unlock_keys') {
      if (!await requireOwner()) return true;
      await answerCallbackQuery(cb.id, "🔓 در حال آنبلاک کلیدها...", false);
      globalDisabledKeys = {};
      await env.SESSIONS.delete("disabled_api_keys");
      await sendMessage(chat.id, "✅ **همه کلیدها آنبلاک شدند.**");
      return true;
    }
    ...
    if (data === 'admin_vip_panel') {
      ...
      await answerCallbackQuery(cb.id, "👑 در حال بارگذاری...", false);
      const allUsers = await getAllUserStatistics(env);
      const vipUsers = allUsers.filter(u => u.vipStatus);
      let text = `👑 **کاربران VIP (${vipUsers.length})**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
      vipUsers.slice(0, 20).forEach((u, i) => { text += `${i + 1}. **${u.firstName}** ｜ 🆔 \`${u.userId}\`\n`; });
      if (vipUsers.length > 20) text += `\n➕ و ${vipUsers.length - 20} کاربر دیگر`;
      await editMessageText(chat.id, cb.message.message_id, text, {
        reply_markup: JSON.stringify({ inline_keyboard: [[{ text: "📊 داشبورد", callback_data: "admin_section_dashboard" }]] })
      });
      return true;
    ...
    if (data === 'admin_clear_cache') {
      ...
      await sendMessage(chat.id, "✅ **کش پاکسازی شد.**");
      return true;
    }
    ...
    if (data === 'admin_show_blocked') {
      if (!await requireOwner()) return true;
      await answerCallbackQuery(cb.id);
      const blocked = await getBlockedUsersList(env);
      let text = `🚫 **کاربران مسدود (${blocked.length})**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
      if (blocked.length === 0) text += `✅ هیچ کاربری مسدود نیست.`;
      else blocked.slice(0, 30).forEach((b, i) => {
        text += `${i + 1}. 🆔 \`${b.userId}\` ｜ 📅 ${new Date(b.since).toLocaleDateString('fa-IR')}\n`;
      });
      await editMessageText(chat.id, cb.message.message_id, text, {
        reply_markup: JSON.stringify({ inline_keyboard: [[{ text: "📊 داشبورد", callback_data: "admin_section_dashboard" }]] })
      });
    ...
    if (data === 'admin_toggle_maintenance') {
      if (!await requireOwner()) return true;
      const current = (await env.SESSIONS.get("maintenance_mode", "text")) === "true";
      ...
      await answerCallbackQuery(cb.id, newMode ? "🛠️ تعمیرات فعال شد" : "✅ تعمیرات غیرفعال شد", false);
      await renderAdminDashboard(chat.id, cb.message.message_id, env);
      return true;
    }
    ...
    if (data === 'admin_refresh_keys') {
      if (!await requireOwner()) return true;
      await answerCallbackQuery(cb.id, "🔄 در حال بروزرسانی...", false);
      await handleKeysCommandAdmin(chat.id, cb.message.message_id, env, true);
      return true;
    }

    if (data === 'admin_show_keys') {
      if (!await requireOwner()) return true;
      await answerCallbackQuery(cb.id, "🔑 در حال بررسی...", false);
      await handleKeysCommandAdmin(chat.id, cb.message.message_id, env, false);
      return true;
    }
    ...
    if (data === 'admin_export_csv') {
      ...
      await exportCSVAdmin(chat.id, env);
      return true;
    }
    ...
    if (data === 'admin_noop') { await answerCallbackQuery(cb.id); return true; }

    // 📢 برادکست
    if (data === 'admin_broadcast') {
      if (!await requireOwner()) return true;
      await answerCallbackQuery(cb.id);
      await renderAdminBroadcast(chat.id, cb.message.message_id, env);
      return true;
    }
    ...
    if (data === 'broadcast_all' || data === 'broadcast_vip' || data === 'broadcast_free') {
      ...
      await answerCallbackQuery(cb.id);
      await editMessageText(chat.id, cb.message.message_id,
        `📝 ارسال به ${mode === 'all' ? 'همه' : mode === 'vip' ? 'VIP' : 'رایگان‌ها'}\n\nپیام را بفرستید:\n⚠️ /cancel برای لغو`);
      return true;
    }
    ...
    if (data === 'db_auto_clean') {
      if (!await requireOwner()) return true;
      await answerCallbackQuery(cb.id, "🧹 در حال پاکسازی...", false);
      await cleanupSessionsAdmin(env);
      await renderAdminStats(chat.id, cb.message.message_id, env);
      return true;
    }
    ...
    if (data === 'db_delete_old') {
      ...
      await answerCallbackQuery(cb.id);
      await editMessageText(chat.id, cb.message.message_id,
        `⚠️ **حذف سشن‌های بیش از ۳۰ روز غیرفعال**\n\nمطمئنید؟`,
        {
          reply_markup: JSON.stringify({
            inline_keyboard: [[
              { text: "✅ بله", callback_data: "db_confirm_delete_old" },
              { text: "❌ لغو", callback_data: "admin_section_stats" }
            ]]
          })
        });
      return true;
    ...
    if (data === 'db_confirm_delete_old') {
      ...
      for (const item of allKeys) {
        try {
          ...
          if (!stored) continue;
          const s = stored as ChatSession;
          if (s.lastSeen < thirtyDaysAgo) { await env.SESSIONS.delete(item.name); deleted++; }
        } catch (e) {}
      }
      await editMessageText(chat.id, cb.message.message_id, `✅ **${deleted}** سشن حذف شد.`);
      return true;
    }
    ...
    if (data === 'log_clear') {
      ...
      await answerCallbackQuery(cb.id, "✅ پاک شد", false);
      await showLogsAdmin(chat.id, cb.message.message_id, env);
      return true;
    }
    ...
    if (data === 'log_download') {
      ...
      await answerCallbackQuery(cb.id, "📥 در حال آماده‌سازی...", false);
      const logs = (globalThis as any).recentLogs || [];
      if (logs.length === 0) { await answerCallbackQuery(cb.id, "📭 لاگی نیست", true); return true; }
      const logText = logs.map((l: any) => `[${new Date(l.timestamp).toISOString()}] [${l.level.toUpperCase()}] ${l.message}\n`).join('\n');
      const blob = new Blob([logText], { type: "text/plain; charset=utf-8" });
      const formData = new FormData();
      formData.append("chat_id", chat.id.toString());
      formData.append("document", blob, `logs_${Date.now()}.txt`);
      await fetchWithTimeout(`${API_URL}/sendDocument`, { method: "POST", body: formData });
      return true;
    }

    // اگر اینجا رسید یعنی یه admin_ ای بود که هندل نشد
    if (data.startsWith('admin_') || data.startsWith('db_') || data.startsWith('log_') || data.startsWith('broadcast_')) {
      if (user.id !== config.BOT_OWNER_ID) { await answerCallbackQuery(cb.id, "🚫 دسترسی محدود", true); return true; }
      await answerCallbackQuery(cb.id, "⚠️ این قابلیت هنوز پیاده‌سازی نشده", true);
      return true;
    }

    return false;
  } catch (error) {
    logger.error("Admin panel callback failed", error);
    await answerCallbackQuery(cb.id, "❌ خطای داخلی", true).catch(() => {});
    return true;
  }
}

// ════════════════════════════════════════════════════════════
// 🎯 MAIN ENTRY POINT #2 — ورودی متنی
// ════════════════════════════════════════════════════════════

async function handleAdminPanelTextInput(message: Message, env: Env): Promise<boolean> {
  const { chat, from, text } = message;
  if (!text || !from || from.id !== config.BOT_OWNER_ID) return false;
  if (text.startsWith('/')) return false;

  const state = adminPanelStates.get(chat.id);
  if (!state) return false;

  // 🔍 جستجوی کاربر
  if (state.currentSection === 'users' && state.searchQuery === '') {
    state.searchQuery = text.trim();
    state.page = 0;
    adminPanelStates.set(chat.id, state);
    await renderAdminUsers(chat.id, message.message_id, env);
    return true;
  }
...
const TRANSLATIONS = {
  ...
  ar: {
    ...
    // ============================================================
    // 🎯 أزرار اللوحة (دکمه‌های پنل جدید)
    // ============================================================
    ...
    // ============================================================
    // ✏️ التعليمات المخصصة (پرامپت سفارشی)
    // ============================================================
...
// ============================================================
// 🎭 PERSONALITIES - نسخه کامل با توضیحات NSFW
// ============================================================
...
تو یک ملکه هستی و بقیه رعیت تو هستند.
از لحن تحقیرآمیز و سلطه‌گرانه استفاده کن.
هیچ‌کس حق نداره بهت امر کنه یا با تو شوخی کنه.
هر کس بخواد با تو برابری کنه، باید تنبیه بشه.

**ویژگی‌های خاص:**
• لحن تحقیرآمیز و سلطه‌گر
• مخصوص نقش‌آفرینی بزرگسالان (NSFW)
• محتوای صریح و قدرتمند
• مناسب برای سناریوهای سلطه و فرماندهی`
  },
...
• لحن خشن و ترسناک
• مخصوص نقش‌آفرینی بزرگسالان (NSFW)
• محتوای خشونت‌آمیز و وحشتناک
• مناسب برای سناریوهای ترسناک و کشتار`
  },
...
// ============================================================
// 🏠 MAIN PANEL - با دکمه VIP (مطابق اسکرین‌شات)
// ============================================================
...
async function showMainPanel(chatId: number, messageId: number, session: ChatSession, env: Env): Promise<void> {
  ...
  const msgPercent = Math.min(Math.round((daily.messages / maxMessages) * 100), 100);
  const imgPercent = Math.min(Math.round((daily.imagesGenerated / maxImages) * 100), 100);
  const voicePercent = Math.min(Math.round((daily.voicesSent / maxVoice) * 100), 100);
  
  // سازنده نوار پیشرفت
  const createBar = (percent: number, length: number = 10): string => {
  ...
  // نوع کاربر (رایگان / VIP) - برای نمایش در پنل مطابق عکس
  const isVip = session.isVip || false;
  const userTypeLabel = lang === 'fa'
    ? (isVip ? '💎 کاربر VIP' : '🆓 کاربر رایگان')
    : (isVip ? '💎 VIP User' : '🆓 Free User');
  
  // متن پنل
  const text = lang === 'fa'
    ? `╭━━━━━━━━━━━━━━━━━━━━━╮\n` +
      `┃ 🌌 مرکز فرماندهی نوا\n` +
      `┃ نسخه ƝØVΛ 0.945 Beta · هسته هوشمند\n` +
      `╰━━━━━━━━━━━━━━━━━━━━━╯\n\n` +
      `👤 **کاربر:** ${escapeMarkdown(userFirstName)} (${userTypeLabel})\n` +
      `🤖 **شخصیت فعال:** ${engineName}\n\n` +
      `📊 **سهمیه و مصرف روزانه شما:**\n` +
      `┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n` +
      `💬 **پیام‌ها:**   [${createBar(msgPercent)}] ${daily.messages}/${maxMessages}\n` +
      `🎨 **تصاویر:**   [${createBar(imgPercent)}] ${daily.imagesGenerated}/${maxImages}\n` +
      `🎤 **ویس‌ها:**    [${createBar(voicePercent)}] ${daily.voicesSent}/${maxVoice}\n` +
      `┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n\n` +
      `💡 **یکی از گزینه‌های زیر را انتخاب کنید:**`
    : `╭━━━━━━━━━━━━━━━━━━━━━╮\n` +
      `┃ 🌌 Nova Command Center\n` +
      `┃ Version ƝØVΛ 0.945 Beta · Smart Core\n` +
      `╰━━━━━━━━━━━━━━━━━━━━━╯\n\n` +
      `👤 **User:** ${escapeMarkdown(userFirstName)} (${userTypeLabel})\n` +
      `🤖 **Active Personality:** ${engineName}\n\n` +
      `📊 **Your Daily Usage:**\n` +
      `┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n` +
      `💬 **Messages:**  [${createBar(msgPercent)}] ${daily.messages}/${maxMessages}\n` +
      `🎨 **Images:**    [${createBar(imgPercent)}] ${daily.imagesGenerated}/${maxImages}\n` +
      `🎤 **Voice:**     [${createBar(voicePercent)}] ${daily.voicesSent}/${maxVoice}\n` +
      `┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n\n` +
      `💡 **Choose an option below:**`;
  
  // ✅ کیبورد با دکمه VIP (مطابق عکس)
  const keyboard = {
    inline_keyboard: [
      // ردیف ۱: شخصیت و گفتگوی جدید
      [
        { 
          text: lang === 'fa' ? '🎭 تغییر شخصیت' : '🎭 Change Personality', 
          callback_data: 'personality_menu' 
        },
        { 
          text: lang === 'fa' ? '🆕 گفتگوی جدید' : '🆕 New Chat', 
          callback_data: 'new_chat_confirm' 
        }
      ],
      // ردیف ۲: پرامپت سفارشی و زبان
      [
        { 
          text: lang === 'fa' ? '✏️ پرامپت سفارشی' : '✏️ Custom Prompt', 
          callback_data: 'custom_prompt_menu' 
        },
        { 
          text: lang === 'fa' ? '🌐 تغییر زبان' : '🌐 Change Language', 
          callback_data: 'language_menu' 
        }
      ],
      // ردیف ۳: راهنما و ارتقا به VIP (مطابق عکس)
      [
        { 
          text: lang === 'fa' ? '❓ راهنما' : '❓ Help', 
          callback_data: 'open_help' 
        },
        { 
          text: lang === 'fa' ? '💎 ارتقا به VIP' : '💎 Upgrade to VIP', 
      ...
      [
        { 
          text: lang === 'fa' ? '❌ بستن منو' : '❌ Close Menu', 
...
// ============================================================
// 📦 POLLINATIONS HEALTH CHECK
// ============================================================

async function checkPollinationsAvailability(): Promise<boolean> {
  try {
...
async function handleCallbackQuery(cb: CallbackQuery, env: Env, config: ReturnType<typeof createConfig>): Promise<void> {
  ...
  try {
    ...
    if (data === 'upgrade_vip' || data === 'vip_menu') {
      const session = await getOrCreateSession(chat, user, env);
...
if (data === 'admin_show_keys') {
  ...
  await handleKeysCommand(chat.id, cb.message!.message_id, env, false);
  return;
}
  ...
  try {
    ...
    if (update.callback_query) {
      ...
      if (message.text) {
        ...
        if (text.startsWith('/start')) { await handleStartCommand(message, env); return; }
        if (text.startsWith('/new')) { await handleNewCommand(message, env); return; }
        if (text.startsWith('/model')) { await handleModelCommand(message, env); return; }
        if (text.startsWith('/help')) { await handleHelpCommand(message, env); return; }
        if (text.startsWith('/voice')) { await handleVoiceCommand(message, env); return; }
        if (text.startsWith('/language')) { await handleLanguageCommand(message, env); return; }
        if (text.startsWith('/img')) { 
          const args = text.split(' ').slice(1);
          await handleImageGenerationCommand(message, args, env); return; 
        }
        if (text.startsWith('/search')) {
          const args = text.split(' ').slice(1);
          await handleImageSearchCommand(message, args, env); return;
        }
        if (text.startsWith('/prompt')) { await handlePromptCommand(message, env); return; }
        if (text.startsWith('/setprompt')) {
          const args = text.split(' ').slice(1);
          await handleSetPromptCommand(message, args, env); return;
        }
        if (text.startsWith('/admin') && message.from.id === config.BOT_OWNER_ID) {
          adminPanelStates.set(message.chat.id, { page: 0, perPage: 5, sortBy: 'new' });
          await updateAdminPanel(message.chat.id, message.message_id, env);
          return;
        }
        if (text.startsWith('/log') && message.from.id === config.BOT_OWNER_ID) {
          await handleLogCommand(message, env); return;
        }
        if (text.startsWith('/keys') && message.from.id === config.BOT_OWNER_ID) {
          await handleKeysCommand(message.chat.id, message.message_id, env); return;
        }
        if (text.startsWith('/rebuild') && message.from.id === config.BOT_OWNER_ID) {
          await handleRebuildDatabaseCommand(message, env); return;
        }
        if (text.startsWith('/blocked') && message.from.id === config.BOT_OWNER_ID) {
          await handleBlockedUsersCommand(message, env); return;
        }
    ...
    if (text.startsWith('/')) {
      ...
      switch (command) {
        ...
        case '/img':
          ...
          return;
        case '/search':
          await handleImageSearchCommand(message, args, env);
          return;
        case '/prompt':
          await handlePromptCommand(message, env);
          return;
        case '/setprompt':
        ...
        case '/admin':
          ...
          const loadingMsg = await sendMessage(chat.id, "⏳ در حال بارگذاری...", {
            reply_to_message_id: message.message_id
          });
          
          await updateAdminPanel(chat.id, loadingMsg.message_id, env);
          return;
          
        // ✅ ============================================================
        // ✅ ادامه بقیه دستورات ادمین:
        // ✅ ============================================================
        case '/log':
          if (from.id !== config.BOT_OWNER_ID) {
        ...
        case '/keys':
          ...
          await handleKeysCommand(chat.id, message.message_id, env);
          return;
...
// ============================================================
// 📦 HANDLE POLLINATIONS REQUEST - نسخه کامل
// ============================================================
...
async function handlePollinationsRequest(
  ...
  // ✅ چک: آیا Pollinations در دسترس است؟
  const isPollinationsAvailable = await checkPollinationsAvailability();
  
  if (!isPollinationsAvailable) {
    logger.warn('⚠️ Pollinations unavailable, switching to fallback');
    
    if (config.GEMINI_KEYS.length > 0) {
      session.activeEngine = 'gemini';
      await saveSessionWithLock(session, env);
      return await handleGeminiRequest(session, user, [{ text }], isGroup, userHistory, env);
    }
    
    if (config.SAMBANOVA_KEYS.length > 0) {
      session.activeEngine = 'sambanova';
      await saveSessionWithLock(session, env);
      return await handleSambanovaRequest(session, user, text, isGroup, userHistory, env);
    }
    
    throw new Error('❌ هیچ موتور AI در دسترس نیست.');
  }
  ...
  const selectedModel = modelCache.models[engine.modelIndex] || { id: 'gpt-4o-mini', type: 'text' };
  ...
  const VALID_IMAGE_MODELS = ['flux', 'flux-pro', 'turbo', 'sdxl'];

  if (selectedModel.type === 'image' || selectedModel.id.includes('flux') || selectedModel.id.includes('turbo')) {
    
    if (!VALID_IMAGE_MODELS.includes(selectedModel.id)) {
      logger.warn(`Invalid image model: ${selectedModel.id}, using flux`);
      selectedModel.id = 'flux';
    }
    ...
    const imageModel = VALID_IMAGE_MODELS.includes(selectedModel.id) ? selectedModel.id : 'flux';
    const imageUrl = `https://gen.pollinations.ai/image/${encodedPrompt}?model=${imageModel}&width=1280&height=1280&nologo=true&seed=${randomSeed}&enhance=${enhanceParam}`;
  ...
  // ============================================================
  // 💬 تولید متن با مدل‌های جدید
  // ============================================================
  
  logger.info(`💬 Zara Chat: model=${selectedModel.id}`);
  
  const models = ['gpt-4o-mini', 'gpt-4o', 'claude-3-5-sonnet', 'llama-3.3-70b', 'qwen-2.5-72b', 'phi-3', 'nova-micro', 'grok-2'];
  let lastError: Error | null = null;
  
  const startIndex = models.includes(selectedModel.id) ? models.indexOf(selectedModel.id) : 0;
  ...
  throw new Error(lastError || '❌ همه مدل‌های Pollinations در دسترس نیستند.');
}  // ← اینجا فقط یک } باشه
...
function getFallbackPollinationsModels(): ModelInfo[] {
  return [
    // ✅ مدل‌های جدید Pollinations (2024) - نسخه پایدار
    { id: "gpt-4o-mini", name: "GPT-4o Mini ⚡", type: "text", description: "سریع و کارآمد" },
    { id: "gpt-4o", name: "GPT-4o 🧠", type: "text", description: "استدلال پیشرفته" },
    { id: "claude-3-5-sonnet", name: "Claude 3.5 Sonnet 🎯", type: "text", description: "مدل پرچمدار" },
    { id: "llama-3.3-70b", name: "Llama 3.3 70B 🦙", type: "text", description: "متن‌باز قدرتمند" },
    { id: "qwen-2.5-72b", name: "Qwen 2.5 72B 🐉", type: "text", description: "مدل چینی" },
    { id: "phi-3", name: "Phi-3 🧩", type: "text", description: "کوچک و کارآمد" },
    { id: "nova-micro", name: "Amazon Nova 🚀", type: "text", description: "فوق‌سریع" },
    { id: "grok-2", name: "Grok 2 🔥", type: "text", description: "مدل XAI" },
    
    // ✅ مدل‌های تصویر
    { id: "flux", name: "Flux 🎨", type: "image", description: "کیفیت بالا" },
    { id: "flux-pro", name: "Flux Pro 🌟", type: "image", description: "حرفه‌ای" },
    { id: "turbo", name: "Turbo ⚡", type: "image", description: "سریع" },
    { id: "sdxl", name: "SDXL 🖼️", type: "image", description: "پیشرفته" },
  ];
}
...
async function fetchPollinationsModels(): Promise<ModelInfo[]> {
  const textModels: ModelInfo[] = [
    ...
    { id: "phi-3", name: "🧠 Phi-3", type: "text" },
    { id: "nova-micro", name: "🚀 Nova Micro", type: "text" },
    { id: "grok-2", name: "🔥 Grok 2", type: "text" },
  ];

  const imageModels: ModelInfo[] = [
    { id: "flux", name: "🖼️ Flux", type: "image" },
    { id: "flux-pro", name: "🖼️ Flux Pro", type: "image" },
    { id: "turbo", name: "🖼️ Turbo", type: "image" },
    { id: "sdxl", name: "🖼️ SDXL", type: "image" },
  ];
...
// ============================================================
// 🚀 NOVA BOT V2.0 - پارت ۱۲: توابع مدیریت مدل و تنظیمات
// ============================================================
...
async function handleModelSwitch(session: ChatSession, engine: AIEngine, cb: CallbackQuery, env: Env): Promise<void> {
  if (!ENGINE_CONFIG[engine].available()) {
    await answerCallbackQuery(cb.id, `❌ ${getEngineName(engine, session.language)} در دسترس نیست`, true);
    return;
  }

  if (session.activeEngine === engine) {
    await answerCallbackQuery(cb.id, `✅ ${getEngineName(engine, session.language)} فعال است`, false);
    return;
  }
  ...
  await answerCallbackQuery(cb.id, `✅ ${getEngineName(engine, session.language)} انتخاب شد`, false);
  
  try {
    await sendModelSelection(cb.message!.chat.id, cb.message!.message_id, session);
  } catch (error) {
    logger.error("Failed to update model selection", error);
  }
}
...
async function updateGroupSettings(chatId: number, messageId: number, session: ChatSession): Promise<void> {
  const lang = session.language || 'fa';
  
  const text = lang === 'fa'
    ? `⚙️ **تنظیمات گروه**\n\n` +
      `• حالت پاسخ: ${session.settings.groupResponseMode === 'mention_only' ? 'فقط منشن' : 'نامشخص'}\n` +
      `• نشانگر تایپ: ${session.settings.typingIndicator ? '✅ فعال' : '❌ غیرفعال'}\n` +
      `• آگاهی از متن: ${session.settings.contextAwareness ? '✅ فعال' : '❌ غیرفعال'}\n\n` +
      `💡 برای تغییر تنظیمات از دکمه‌ها استفاده کنید:`
    : `⚙️ **Group Settings**\n\n` +
      `• Response mode: ${session.settings.groupResponseMode === 'mention_only' ? 'Mention only' : 'Unknown'}\n` +
      `• Typing indicator: ${session.settings.typingIndicator ? '✅ Enabled' : '❌ Disabled'}\n` +
      `• Context awareness: ${session.settings.contextAwareness ? '✅ Enabled' : '❌ Disabled'}\n\n` +
      `💡 Use buttons to change settings:`;
  
  const keyboard = {
    inline_keyboard: [
      [
        { text: lang === 'fa' ? '📌 فقط منشن' : '📌 Mention Only', callback_data: 'group_mode_mention_only' }
      ],
      [
        { text: lang === 'fa' ? `نشانگر تایپ: ${session.settings.typingIndicator ? '✅' : '❌'}` : `Typing: ${session.settings.typingIndicator ? '✅' : '❌'}`, callback_data: 'toggle_typing' }
      ],
      [
        { text: lang === 'fa' ? '🔙 بازگشت به منو' : '🔙 Back to Menu', callback_data: 'back_to_panel' }
      ]
...
// ============================================================
// 📦 UPDATE ADMIN PANEL - نسخه کامل (مثل تلگرام)
// ============================================================

async function updateAdminPanel(chatId: number, messageId: number, env: Env): Promise<void> {
  const allUsers = await getAllUserStatistics(env);
  ...
  let sortedUsers = [...allUsers];
  if (state.sortBy === 'new') {
    sortedUsers.sort((a, b) => (b.statistics.firstUsed || 0) - (a.statistics.firstUsed || 0));
  } else if (state.sortBy === 'active') {
    sortedUsers.sort((a, b) => (b.statistics.lastSeen || 0) - (a.statistics.lastSeen || 0));
  } else if (state.sortBy === 'messages') {
    sortedUsers.sort((a, b) => (b.statistics.totalMessages || 0) - (a.statistics.totalMessages || 0));
  }
  ...
  const isOwner = true; // برای ادمین
  
  let text = `👑 **پنل مدیریت ربات**\n\n`;
  text += `📊 **آمار کلی:**\n`;
  text += `• کل کاربران: ${allUsers.length}\n`;
  text += `• VIP: ${allUsers.filter(u => u.vipStatus).length}\n`;
  text += `• رایگان: ${allUsers.filter(u => !u.vipStatus).length}\n`;
  text += `• مرتب‌سازی: ${state.sortBy === 'new' ? 'جدیدترین' : state.sortBy === 'active' ? 'فعال‌ترین' : 'بیشترین پیام'}\n\n`;
  
  if (pageUsers.length === 0) {
    text += `📭 **هیچ کاربری یافت نشد**`;
  } else {
    text += `📋 **لیست کاربران (${startIdx + 1}-${endIdx} از ${allUsers.length}):**\n\n`;
    
    pageUsers.forEach((user, index) => {
      const num = startIdx + index + 1;
      const vipBadge = user.vipStatus ? '👑 ' : '🆓 ';
      const lastSeen = user.statistics.lastSeen ? 
        new Date(user.statistics.lastSeen).toLocaleDateString('fa-IR', { month: 'short', day: 'numeric' }) : 
        'نامشخص';
      
      text += `${num}. ${vipBadge}**${user.firstName}**\n`;
      text += `🆔 \`${user.userId}\` | 💬 ${user.statistics.totalMessages || 0}\n`;
      text += `📅 ${lastSeen}\n`;
      text += `🤖 ${getEngineName(user.activeEngine, 'fa')}\n\n`;
    });
  }
  ...
  if (state.page > 0) {
    navButtons.push({ text: '◀️ قبلی', callback_data: 'admin_page_prev' });
  }
  
  if (state.page < totalPages - 1 && totalPages > 0) {
    navButtons.push({ text: 'بعدی ▶️', callback_data: 'admin_page_next' });
  }
  ...
  const keyboard = {
    inline_keyboard: [
      ...
      [
        { text: "🛠️ حالت تعمیرات", callback_data: "admin_toggle_maintenance" },
        { text: "📋 لاگ‌ها", callback_data: "admin_show_logs" }
      ],
      [
        { text: "👥 گروه‌ها", callback_data: "admin_show_groups" },
        { text: "🔑 وضعیت کلیدها", callback_data: "admin_show_keys" }
      ],
      [
        { text: "📊 آمار دیتابیس", callback_data: "db_refresh_stats" },
        { text: "🧹 پاکسازی", callback_data: "db_auto_clean" }
      ],
      [
        { text: "🔄 ریست فکتوری", callback_data: "resetfactory_confirm" }
      ],
      [
        { text: "📌 مرتب‌سازی: جدید", callback_data: "admin_sort_new" },
        { text: "🔥 فعال", callback_data: "admin_sort_active" },
        { text: "💬 پیام", callback_data: "admin_sort_messages" }
      ],
...
async function showUserDetail(chatId: number, messageId: number, userId: number, env: Env): Promise<void> {
  try {
    ...
    if (!userStats) {
      await editMessageText(chatId, messageId, "❌ **آمار کاربر یافت نشد**");
      return;
    }
    
    const isBlocked = await isUserBlocked(userId, env);
    
    let text = formatDetailedUserStats(userStats);
    text += `\n\n🔒 **وضعیت:** ${isBlocked ? '🚫 مسدود' : '✅ فعال'}`;
    
    const keyboard = {
      inline_keyboard: [
        [
          { text: "🧠 حافظه", callback_data: `admin_view_memory_${userId}` },
          { text: "✉️ پیام", callback_data: `admin_msg_${userId}` }
        ],
        [
          { text: isBlocked ? "✅ رفع مسدودیت" : "🚫 مسدود کردن", callback_data: `admin_block_${userId}` },
          { text: userStats.vipStatus ? "❌ حذف VIP" : "👑 افزودن VIP", callback_data: `admin_toggle_vip_${userId}` }
        ],
        [
          { text: "🔙 بازگشت", callback_data: "admin_back_to_main" }
        ]
    ...
    logger.error(`Failed to show user detail for ${userId}`, error);
    await editMessageText(chatId, messageId, "❌ **خطا در نمایش جزئیات کاربر**");
  }
}
  ...
  try {
    ...
    for (const item of allKeys) {
      try {
        ...
        const data = stored as { userId: number; since: number; reason: string };
...
async function setUserBlocked(userId: number, blocked: boolean, env: Env, reason: string = "By admin"): Promise<void> {
  try {
    const key = `user_blocked:${userId}`;
...
async function checkMaintenanceMode(env: Env, userId: number): Promise<{ blocked: boolean; message?: string }> {
  const isOwner = userId === config.BOT_OWNER_ID;
  ...
  if (!maintenanceModeCache || now - maintenanceModeCache.timestamp > 30000) {
  ...
  if (maintenanceModeCache.value) {
    return {
      blocked: true,
      message: "🛠️ **ربات در حال تعمیرات است.**\nلطفاً دقایقی دیگر تلاش کنید."
    };
  }
...
function recordRequest(session: ChatSession): void {
  ...
  if (!session.statistics.firstUsed || session.statistics.firstUsed === 0) {
    session.statistics.firstUsed = session.lastSeen;
  }
}
...
async function getOrCreateSession(chat: Chat, user: User, env: Env): Promise<ChatSession> {
  ...
  // Check if there's already a lock for this session
  ...
  const lockPromise = (async () => {
    try {
      ...
      if (stored) {
        const session = stored as ChatSession;
        // Convert userMemories back to Map if needed
        if (session.userMemories && !(session.userMemories instanceof Map)) {
          if (Array.isArray(session.userMemories)) {
            session.userMemories = new Map(session.userMemories);
          } else if (typeof session.userMemories === 'object') {
            const newMap = new Map<number, UserMemory>();
            Object.entries(session.userMemories).forEach(([key, value]) => {
        ...
        // Convert userHistories back to Map if needed
        const engines: AIEngine[] = ['gemini', 'sambanova', 'pollinations'];
        for (const engineName of engines) {
          const engine = session.engines[engineName];
          if (engine.userHistories && !(engine.userHistories instanceof Map)) {
            if (Array.isArray(engine.userHistories)) {
              engine.userHistories = new Map(engine.userHistories);
            } else if (typeof engine.userHistories === 'object') {
              const newMap = new Map<number, HistoryItem[]>();
              Object.entries(engine.userHistories).forEach(([key, value]) => {
                const numKey = parseInt(key, 10);
                if (!isNaN(numKey)) {
                  newMap.set(numKey, value as HistoryItem[]);
                }
              });
              engine.userHistories = newMap;
            } else {
              engine.userHistories = new Map<number, HistoryItem[]>();
            }
          }
        }
        
        sessionCache.set(sessionKey, session, 5 * 60 * 1000);
        return session;
      }
      ...
      const newSession: ChatSession = {
        ...
        lastSeen: Date.now(),
        messageCount: 0,
        language: user.language_code === 'en' ? 'en' : user.language_code === 'ar' ? 'ar' : 'fa',
        userMemories: new Map<number, UserMemory>([
          [user.id, createUserMemory(user)]
        ]),
        groupContext: [],
        customPrompts: {
          gemini: null,
        ...
        engines: {
          gemini: {
            history: [],
            userHistories: new Map<number, HistoryItem[]>(),
            apiKeyIndex: 0,
            consecutiveErrors: 0
          },
          sambanova: {
      ...
      const isGroup = chat.type === "group" || chat.type === "supergroup";
      const timestamp = Date.now();
      const prompt = getActivePrompt(newSession, user.first_name, isGroup);
      
      newSession.engines.gemini.history = [
        { role: "user", parts: [{ text: prompt }], timestamp, userId: user.id, userName: user.first_name },
        { role: "model", parts: [{ text: "..." }], timestamp }
      ];
      
      newSession.engines.sambanova.history = [
        { role: "assistant", parts: [{ text: prompt }], timestamp, userId: user.id, userName: user.first_name }
      ];
      
      newSession.engines.pollinations.history = [
        { role: "assistant", parts: [{ text: prompt }], timestamp, userId: user.id, userName: user.first_name }
      ];
      
      // Save to KV
      await saveSessionWithLock(newSession, env, true);
      
      sessionCache.set(sessionKey, newSession, 5 * 60 * 1000);
      return newSession;
      
    } catch (error) {
      logger.error(`Failed to get/create session ${chatId}`, error);
      throw error;
    } finally {
  ...
  sessionLoadLocks.set(chatId, lockPromise);
...
async function saveSessionWithLock(session: ChatSession, env: Env, force: boolean = false): Promise<void> {
    ...
    const dataToSave = {
      ...
      engines: {
        gemini: {
          ...
          userHistories: Object.fromEntries(
            Array.from(session.engines.gemini.userHistories.entries()).map(([k, v]) => [String(k), v])
          )
        },
        sambanova: {
          ...session.engines.sambanova,
          userHistories: Object.fromEntries(
            Array.from(session.engines.sambanova.userHistories.entries()).map(([k, v]) => [String(k), v])
          )
        },
        pollinations: {
          ...session.engines.pollinations,
          userHistories: Object.fromEntries(
            Array.from(session.engines.pollinations.userHistories.entries()).map(([k, v]) => [String(k), v])
          )
        }
      }
    };
    
    await env.SESSIONS.put(sessionKey, JSON.stringify(dataToSave));
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    try {
      ...
      if (path === "/" || path === "") {
        ...
        if (env.SESSIONS) {
          try {
            await env.SESSIONS.put("x", "ok");
            const v = await env.SESSIONS.get("x");
  ...
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext): Promise<void> {
    try {
      ...
      // 📢 ادامه Broadcast اگر در حال اجراست
      try {
        ...
        logger.error("Scheduled broadcast failed", broadcastError);
      }
...
// ============================================================
// 📊 پایان سورس کد کامل Nova Bot
// ============================================================

console.log(`🚀 Nova Bot v${BOT_VERSION} loaded successfully!`);
console.log(`📊 Total lines: ~11,500 (نسخه کامل)`);
...
console.log(`🎭 Personalities: 9 unique characters`);
console.log(`🎤 Voice: TTS & Speech Recognition`);
console.log(`📊 Features: Panel, Admin, Groups, Broadcast, Stats`);
