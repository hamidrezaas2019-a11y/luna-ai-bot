...
async function sendMainMenu(message: Message, session: ChatSession, env: Env): Promise<void> {
  ...
  const text = lang === 'fa' ?
    `🚀 **مرکز فرماندهی نوآ**\n━━━━━━━━━━━━━━━━━━━━\n` +
    `👤 کاربر: ${message.from?.first_name || 'کاربر'} ${session.vipStatus ? '💎 VIP' : ''}\n` +
    `🎭 شخصیت: ${personalityEmoji} ${personalityName}\n` +
    `🤖 مدل: ${engineName}\n` +
    `━━━━━━━━━━━━━━━━━━━━\n` +
    `📊 **مصرف روزانه:**\n` +
    ...
    `🚀 **Nova Command Center**\n━━━━━━━━━━━━━━━━━━━━\n` +
    `👤 User: ${message.from?.first_name || 'User'} ${session.vipStatus ? '💎 VIP' : ''}\n` +
    `🎭 Personality: ${personalityEmoji} ${personalityName}\n` +
    `🤖 Model: ${engineName}\n` +
    `━━━━━━━━━━━━━━━━━━━━\n` +
    `📊 **Daily Usage:**\n` +
  ...
  const keyboard = isGroup ? {
    inline_keyboard: [
      [{ text: '👥 تنظیمات گروه', callback_data: 'group_settings' }],
      [{ text: '❌ بستن', callback_data: 'close' }]
    ]
  } : {
    inline_keyboard: [
      [{ text: '🤖 مدل‌ها', callback_data: 'model_settings' }, { text: '🎭 شخصیت‌ها', callback_data: 'personality_menu' }],
      [{ text: '❓ راهنما', callback_data: 'help_menu' }, { text: '⭐ VIP', callback_data: 'menu_vip' }],
      [{ text: '❌ بستن', callback_data: 'close' }]
    ]
  };
  ...
  const keyboard = {
    inline_keyboard: [
      ...
      [
        ...
        { text: `${PERSONALITIES.shadow.emoji} شادو`, callback_data: 'set_personality_shadow' }
      ],
      [
...
async function sendModelMenu(message: Message, session: ChatSession, env: Env): Promise<void> {
  ...
  const text = lang === 'fa' ?
    `🤖 **انتخاب مدل**\n━━━━━━━━━━━━━━━━━━━━\n` +
    `مدل فعلی: ${getEngineName(currentEngine, lang)}\n\n` +
    `یکی از مدل‌های زیر را انتخاب کنید:` :
    `🤖 **Select Model**\n━━━━━━━━━━━━━━━━━━━━\n` +
    `Current model: ${getEngineName(currentEngine, lang)}\n\n` +
    `Select one of the models below:`;
  
  const keyboard = {
    inline_keyboard: [
      [
        { text: `${currentEngine === 'gemini' ? '✅ ' : ''}🤖 نوا (Gemini)`, callback_data: 'set_model_gemini' }
      ],
      [
        { text: `${currentEngine === 'sambanova' ? '✅ ' : ''}🧠 لونا (SambaNova) ${!session.vipStatus ? '🔒' : ''}`, callback_data: session.vipStatus ? 'set_model_sambanova' : 'vip_only' }
      ],
      [
        { text: `${currentEngine === 'pollinations' ? '✅ ' : ''}✨ زارا (Pollinations)`, callback_data: 'set_model_pollinations' }
      ],
      [
        { text: '🔙 بازگشت', callback_data: 'back_to_main_menu' }
      ]
