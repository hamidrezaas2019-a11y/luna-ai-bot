  ...
  async fetch(
    ...
    if (request.method === "POST" && path === "/webhook") {
      try {
        ...
        ctx.waitUntil(
          handleUpdate(update, env, config)
            .catch(err => {
    ...
    try {
      ...
      // ۱. تمیزکاری حافظه مدل‌ها
      if (modelListStates.size > 100) {
        modelListStates.clear();
      }

      // ۲. تمیزکاری کانتکست گروه‌ها
      for (const [chatId, context] of groupContextCache.entries()) {
      ...
      // ۳. اینجا اگه کار دیگه‌ای (مثل برودکست) داری بذار
      // حتماً هر بلاکی که باز می‌کنی رو با } ببند

      logger.info("✅ Scheduled cleanup completed");

    } catch (error) {
      logger.error("Scheduled failed", error);
    }
  }
