...
// ============================================================
// 🏠 MAIN PANEL - پنل اصلی (نسخه جدید با ظاهر زیبا)
// ============================================================
...
async function showMainPanel(chatId: number, messageId: number, session: ChatSession, env: Env): Promise<void> {
  ...
  const msgPercent = Math.min(Math.round((daily.messages / 50) * 100), 100);
  const imgPercent = Math.min(Math.round((daily.imagesGenerated / 7) * 100), 100);
  const voicePercent = Math.min(Math.round((daily.voicesSent / 3) * 100), 100);
  ...
  const text = lang === 'fa'
    ? `╭━━━━━━━━━━━━━━━━━━━━━╮\n` +
      ...
      `╰━━━━━━━━━━━━━━━━━━━━━╯\n\n` +
      `👤 کاربر: ${userFirstName} (${isVip ? '👑 کاربر ویژه' : '🆓 کاربر رایگان'})\n` +
      `🤖 شخصیت فعال: ${personalityEmoji} ${personalityName || 'نوا'} — ${personalityDesc || 'دستیار هوشمند'}\n\n` +
      `📊 سهمیه و مصرف روزانه شما:\n` +
      `┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n` +
      `💬 پیام‌ها:   [${'█'.repeat(Math.floor(msgPercent/10))}${'▱'.repeat(10 - Math.floor(msgPercent/10))}] ${daily.messages}/${maxMessages}\n` +
      `🎨 تصاویر:   [${'█'.repeat(Math.floor(imgPercent/10))}${'▱'.repeat(10 - Math.floor(imgPercent/10))}] ${daily.imagesGenerated}/${maxImages}\n` +
      `🎤 ویس‌ها:    [${'█'.repeat(Math.floor(voicePercent/10))}${'▱'.repeat(10 - Math.floor(voicePercent/10))}] ${daily.voicesSent}/${maxVoice}\n` +
      `┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n\n` +
    ...
    : `╭━━━━━━━━━━━━━━━━━━━━━╮\n` +
      ...
      `╰━━━━━━━━━━━━━━━━━━━━━╯\n\n` +
      `👤 User: ${userFirstName} (${isVip ? '👑 VIP User' : '🆓 Free User'})\n` +
      `🤖 Active Personality: ${personalityEmoji} ${personalityName || 'Nova'} — ${personalityDesc || 'Smart Assistant'}\n\n` +
      `📊 Your Daily Usage:\n` +
      `┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n` +
      `💬 Messages:  [${'█'.repeat(Math.floor(msgPercent/10))}${'▱'.repeat(10 - Math.floor(msgPercent/10))}] ${daily.messages}/${maxMessages}\n` +
      `🎨 Images:    [${'█'.repeat(Math.floor(imgPercent/10))}${'▱'.repeat(10 - Math.floor(imgPercent/10))}] ${daily.imagesGenerated}/${maxImages}\n` +
      `🎤 Voice:     [${'█'.repeat(Math.floor(voicePercent/10))}${'▱'.repeat(10 - Math.floor(voicePercent/10))}] ${daily.voicesSent}/${maxVoice}\n` +
      `┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n\n` +
