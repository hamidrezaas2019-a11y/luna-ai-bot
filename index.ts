...
// ============================================================
// 4️⃣ بخش چهارم: EXPORT DEFAULT (آخرین بخش)
// ============================================================
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    try {
      ...
      if (url.pathname === '/webhook' && request.method === 'POST') {
        if (!isInitialized) {
          await initializeBot(env, config);
          isInitialized = true;
        }

        try {
          const body = await request.json() as Update;
          // ✅ استفاده از ctx برای اجرای پس‌زمینه
          ctx.waitUntil(handleUpdate(body, env, config));
      ...
      if (url.pathname === '/trigger-next-batch') {
        ctx.waitUntil(processBroadcastBatch(env));
        return new Response('OK', { status: 200 });
      }

      return new Response('Nova AI Bot is running! 🚀', {
        headers: { 'Content-Type': 'text/plain' }
      });

    } catch (error) {
      logger.error('Fatal error in fetch:', error);
