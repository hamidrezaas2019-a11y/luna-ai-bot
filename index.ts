...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    // تست دستی وضعیت دیتابیس (روت اصلی وب‌سایت)
    if (path === "/" || path === "") {
    ...
    // پردازش وب‌هوک دریافتی از پیام‌رسان
  ...
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext): Promise<void> {
    try {
      ...
      // مدیریت ارسال پیام‌های همگانی زمان‌بندی‌شده
      try {
        ...
        if (
          ...
          broadcastJob.processedIndex < broadcastJob.totalUsers
        ) {
