...
// ============================================================
// 🚀 NOVA BOT - نسخه نهایی (فقط یک export default)
// ============================================================
...
async function initializeBot(env, cfg) {
  ...
  logger.info(`🔑 TOKEN: ${cfg.TOKEN ? '✅ Set' : '❌ Missing'}`);
...
async function handleUpdate(update, env, cfg) {
  logger.info(`📩 Update: ${update.update_id}`);
  
  if (update.message) {
    const chatId = update.message.chat.id;
    const text = update.message.text || '';
    
    logger.info(`💬 From ${chatId}: ${text}`);
