...
// ذخیره گروه هنگام join شدن ربات
async function onBotJoinedGroup(chat: any, source: string, env: any) {
...
// حذف گروه هنگام leave شدن ربات
async function onBotLeftGroup(chatId: number, env: any) {
  let groups: Group[] = await env.SESSIONS.get("joined_groups", "json") || [];
  groups = groups.filter(g => g.id !== chatId);
  await env.SESSIONS.put("joined_groups", JSON.stringify(groups));
}

// نمایش پنل مدیریت گروه‌ها
async function showActiveGroups(chatId: number, msgId: number, env: any) {
  const groups: Group[] = await env.SESSIONS.get("joined_groups", "json") || [];

  let text = `👥 **ACTIVE GROUPS MANAGER**\n`;
  text += `━━━━━━━━━━━━━━━━━━━━\n\n`;
  text += `**Total Joined Groups:** ${groups.length}\n\n`;

  if (groups.length === 0) {
    text += `_هیچ گروهی یافت نشد_`;
  } else {
    groups.forEach((g, i) => {
      const date = new Date(g.joinedAt).toLocaleDateString('en-US', {
        month: 'numeric',
        day: 'numeric',
        year: 'numeric'
      });
      text += `${i + 1}. **${g.title}**\n`;
      text += `↳ Source: \`${g.source}\` | Active: ${date}\n\n`;
    });
  }
...
// نمایش جزئیات یک گروه خاص
async function showGroupDetail(chatId: number, msgId: number, groupId: number, env: any) {
  const groups: Group[] = await env.SESSIONS.get("joined_groups", "json") || [];
  ...
  const text = `📊 **Group Details**\n\n` +
    ...
    `**ID:** \`${group.id}\`\n` +
    `**Source:** ${group.source}\n` +
    `**Joined:** ${new Date(group.joinedAt).toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'long',
      day: 'numeric'
    })}`;
  ...
  async function handleCallbackQuery(cb: CallbackQuery, env: Env, config: ReturnType<typeof createConfig>) {
  try {
    answerCallbackQuery(cb.id).catch(() => {});
  } catch (e) {}
  ...
  try {
    ...
    const userId = cb.from.id;
    const now = Date.now();
    if (!callbackRateLimits.has(userId)) callbackRateLimits.set(userId, []);
    const userCallbacks = callbackRateLimits.get(userId)!;
    const recent = userCallbacks.filter(t => now - t < 10000);
    if (recent.length >= 15 && userId !== config.BOT_OWNER_ID) {
      await answerCallbackQuery(cb.id, "⏳ خیلی سریع! یکم صبر کن", true);
    ...
    // ✅ اگر اولین بار است که data رو تعریف کردی، همین خوب است
// ✅ حالا if رو اینجا قرار بده (قبل از switch)
if (data.startsWith("grp_") || data.startsWith("leave_grp_") || data === "groups_refresh") {
  await handleGroupsCallback(cb, env);
  return;
}
    
    // ادامه کدهای قبلی مثل set_lang_ و ... (همونی که قبلاً بود)
    // ... (بقیه کدها)
...
// استفاده در handleCallbackQuery اصلی:
// if (data.startsWith("grp_") || data.startsWith("leave_grp_") || data === "groups_refresh") {
//   await handleGroupsCallback(cb, env);
//   return;
// }
...
handleLogCommand(message: Message, env: Env): Promise<void> {
  const { chat } = message;
...
export default {
  ...
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext): Promise<void> {
    // ... (کد scheduled شما که قبلاً داشتید، بدون تغییر)
  }
};
