...
// ربات نوآ (NOVA Agent) - نسخه کامل نهایی با ۱۰ شخصیت
// ورژن 3.0.0 - FULL VERSION
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/setwebhook') {
      ...
      const result = await setWebhook(env.BOT_TOKEN, webhookUrl);
      return new Response(result ? `✅ Webhook تنظیم شد: ${webhookUrl}` : '❌ خطا', { status: 200 });
    }
    ...
    return new Response('🤖 ربات NOVA - فعال ✅\nورژن 3.0.0 - ۱۰ شخصیت', { 
      status: 200, 
...
async function sendRequest(method, params, env) {
...
async function sendMessage(chatId, text, keyboard = null, parseMode = "HTML", env) {
  const params = { chat_id: chatId, text, parse_mode: parseMode };
  if (keyboard) params.reply_markup = JSON.stringify(keyboard);
  return await sendRequest("sendMessage", params, env);
}

async function sendPhoto(chatId, photo, caption = "", keyboard = null, env) {
  const params = { chat_id: chatId, photo, caption, parse_mode: "HTML" };
  if (keyboard) params.reply_markup = JSON.stringify(keyboard);
  return await sendRequest("sendPhoto", params, env);
}

async function sendDocument(chatId, document, caption = "", env) {
  const params = { chat_id: chatId, document, caption };
  return await sendRequest("sendDocument", params, env);
}

async function sendVoice(chatId, voice, caption = "", env) {
  const params = { chat_id: chatId, voice, caption };
  return await sendRequest("sendVoice", params, env);
}

async function deleteMessage(chatId, messageId, env) {
  await sendRequest("deleteMessage", { chat_id: chatId, message_id: messageId }, env);
}

async function answerCallbackQuery(callbackQueryId, text, env) {
  await sendRequest("answerCallbackQuery", { callback_query_id: callbackQueryId, text }, env);
}
