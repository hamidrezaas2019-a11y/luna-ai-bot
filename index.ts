...
Always look for beauty in everything.`
  ...
  // ============================================================
  // 🆕 شخصیت‌های جدید (بیگانه و ویکتوریا)
  // ============================================================
  alien: {
    emoji: "👽",
    name_fa: "بیگانه",
...
function canProcessConcurrentRequest(chatId: number, requestId: string): boolean {
  ...
  // پاکسازی درخواست‌های قدیمی (بیشتر از ۲ دقیقه)
  const now = Date.now();
...
// ادامه بقیه کد بدون تغییر باقی می‌ماند.
...
async function getModelsWithCache(engine: AIEngine, env: Env, forceRefresh: boolean = false): Promise<ModelCache> {
  ...
  if (cached && !forceRefresh) {
    ...
    // اگر کش کمتر از 12 ساعت است، برگردان
  ...
  // کش منقضی شده یا وجود ندارد - fetch جدید
...
function buildMainHelpPage(session: ChatSession, user: User, isGroup: boolean): string {
  const lang  = session.language || 'fa';
  const m     = MODEL_META;
  const mName = lang === 'fa' ? m[session.activeEngine].fa : m[session.activeEngine].en;
  ...
  if (lang === 'fa') {
    return (
      ...
      `> 🤖 موتور فعال: **${m[session.activeEngine].emoji} ${mName}**\n` +
      `> 💳 وضعیت: **${vip}**\n\n` +
  ...
  return (
    ...
    `> 🤖 Engine: **${m[session.activeEngine].emoji} ${mName}**\n` +
    `> 💳 Status: **${vip}**\n\n` +
...
// ============================================================
// 🎯 پنل‌های UI (بر اساس اسکرین‌شات‌ها)
// ============================================================

// ======================== پنل ۱: مرکز فرماندهی نوا ========================
async function showMainPanel(chatId: number, messageId: number, session: ChatSession): Promise<void> {
  const lang = session.language || 'fa';
  const user = session.userMemories.values().next().value;
  const userName = user?.firstName || 'کاربر';
  const isVip = session.vipStatus;
  const daily = session.dailyLimits;
  
  // محاسبه میزان مصرف به صورت نوار پیشرفت
  const messageProgress = Math.min(daily.messages / 50 * 100, 100);
  const voiceProgress = Math.min(daily.voicesSent / 7 * 100, 100);
  const imageProgress = Math.min(daily.imagesGenerated / 3 * 100, 100);
  ...
  const text = lang === 'fa' 
    ? `╭━━━━━━━━━━━━━━━━━━━━━╮
┃ 🌌 **مرکز فرماندهی نوا**
┃ نسخه ${BOT_VERSION} · هسته هوشمند
╰━━━━━━━━━━━━━━━━━━━━━╯

👤 کاربر: **${userName}** (${isVip ? '👑 VIP' : '🆓 کاربر رایگان'})
🤖 شخصیت فعال: **${session.activePersonality ? PERSONALITIES[session.activePersonality]?.name_fa : 'نوا'}** — ${session.activePersonality ? PERSONALITIES[session.activePersonality]?.desc_fa : 'دستیار هوشمند'}

📊 **سهمیه و مصرف روزانه شما:**
┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄
💬 پیام‌ها:   ${progressBar(messageProgress)} ${daily.messages}/50
🎨 تصاویر:   ${progressBar(imageProgress)} ${daily.imagesGenerated}/3
🎤 ویس‌ها:    ${progressBar(voiceProgress)} ${daily.voicesSent}/7
┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄

💡 **یکی از گزینه‌های زیر را انتخاب کنید:**`
    : `╭━━━━━━━━━━━━━━━━━━━━━╮
┃ 🌌 **Nova Command Center**
┃ Version ${BOT_VERSION} · Smart Core
╰━━━━━━━━━━━━━━━━━━━━━╯

👤 User: **${userName}** (${isVip ? '👑 VIP' : '🆓 Free User'})
🤖 Active Personality: **${session.activePersonality ? PERSONALITIES[session.activePersonality]?.name_en : 'Nova'}** — ${session.activePersonality ? PERSONALITIES[session.activePersonality]?.desc_en : 'Smart Assistant'}

📊 **Daily Usage:**
┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄
💬 Messages:   ${progressBar(messageProgress)} ${daily.messages}/50
🎨 Images:     ${progressBar(imageProgress)} ${daily.imagesGenerated}/3
🎤 Voices:     ${progressBar(voiceProgress)} ${daily.voicesSent}/7
┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄
  ...
  const keyboard = {
    inline_keyboard: [
      [
        ...
        { text: lang === 'fa' ? '🆕 گفتگوی جدید' : '🆕 New Chat', callback_data: 'new_chat' }
      ],
      [
        { text: lang === 'fa' ? '✏️ پرامپت سفارشی' : '✏️ Custom Prompt', callback_data: 'custom_prompt_menu' },
        { text: lang === 'fa' ? '🌐 تغییر زبان' : '🌐 Change Language', callback_data: 'change_language' }
      ],
      [
      ...
      [
        { text: lang === 'fa' ? '❌ بستن منو' : '❌ Close Menu', callback_data: 'close_menu' }
      ]
    ]
  };
...
// ======================== پنل ۲: انتخاب شخصیت ========================
async function showPersonalityPanel(chatId: number, messageId: number, session: ChatSession): Promise<void> {
  const lang = session.language || 'fa';
  const currentPersonality = session.activePersonality || 'nova';
  const current = PERSONALITIES[currentPersonality];
  const currentName = lang === 'fa' ? current?.name_fa : current?.name_en;
  const currentEmoji = current?.emoji || '🤖';
  
  // ترتیب شخصیت‌ها بر اساس اسکرین‌شات
  const personalityOrder = [
    'nova', 'lilith', 'alien', 'victoria', 'aria', 'jax', 'luna', 'zara'
  ];

  const text = lang === 'fa'
    ? `🎭 **انتخاب شخصیت**
━━━━━━━━━━━━━━━━━━━━
فعال: **${currentEmoji} ${currentName}** — ${current?.desc_fa || 'دستیار هوشمند'}

هر شخصیت لحن، رفتار و تخصص متفاوتی دارد.
برای تغییر یکی رو انتخاب کن:`
    : `🎭 **Select Personality**
━━━━━━━━━━━━━━━━━━━━
Active: **${currentEmoji} ${currentName}** — ${current?.desc_en || 'Smart Assistant'}

Each personality has different tone, behavior and expertise.
Tap to change:`;

  // ساخت دکمه‌ها در ۲ ردیف
  const buttons = [];
  let row = [];
  ...
  for (const key of personalityOrder) {
    ...
    const label = `${p.emoji} ${lang === 'fa' ? p.name_fa : p.name_en}`;
    const callback = isActive ? 'personality_already_active' : `set_personality_${key}`;
    
    row.push({ text: isActive ? `✅ ${label}` : label, callback_data: callback });
  ...
  // اضافه کردن دکمه پارامتر دستی و بازگشت
  buttons.push([
    { text: lang === 'fa' ? '✏️ پرامپت دستی' : '✏️ Custom Prompt', callback_data: 'custom_prompt_menu' },
    { text: lang === 'fa' ? '🔙 بازگشت' : '🔙 Back', callback_data: 'back_to_main_panel' }
  ]);
...
async function resetUserMemory(chatId: number, messageId: number, userId: number, env: Env): Promise<void> {
  try {
    ...
    // ریست کردن تاریخچه تمام موتورها
    const engines: AIEngine[] = ['gemini', 'sambanova', 'pollinations'];
    const timestamp = Date.now();
...
// ============================================================
// 📦 تابع نمایش منوی شخصیت‌ها
// ============================================================

async function sendPersonalityMenu(chatId: number, messageId: number, session: ChatSession): Promise<void> {
  const lang = session.language || 'fa';
  const currentPersonality = session.activePersonality || 'nova';
  
  // دریافت نام شخصیت فعلی
  const current = PERSONALITIES[currentPersonality];
  const currentName = lang === 'fa' 
    ? current?.name_fa || 'نوا'
    : current?.name_en || 'Nova';
  
  const currentEmoji = current?.emoji || '🤖';
  const currentDesc = lang === 'fa' ? current?.desc_fa : current?.desc_en;

  // ✅ ترتیب جدید با ۱۰ شخصیت
  const personalityOrder = [
    'nova', 'lilith', 'alien', 'victoria', 
    'aria', 'jax', 'luna', 'zara', 
    'cipher', 'leatherface'
  ];
  ...
  const text = lang === 'fa'
    ? `🎭 **انتخاب شخصیت ربات**
━━━━━━━━━━━━━━━━━━━━
فعال: **${currentEmoji} ${currentName}** — ${currentDesc || 'دستیار هوشمند'}

هر شخصیت لحن، رفتار و تخصص متفاوتی دارد.
برای تغییر یکی رو انتخاب کن:`
    : `🎭 **Select Bot Personality**
━━━━━━━━━━━━━━━━━━━━
Active: **${currentEmoji} ${currentName}** — ${currentDesc || 'Smart Assistant'}
...
Tap to change:`;
  ...
  // اضافه کردن دکمه پرامپت دستی و بازگشت
  buttons.push([
    { text: lang === 'fa' ? '✏️ پرامپت دستی' : '✏️ Custom Prompt', callback_data: 'custom_prompt_menu' },
    { text: lang === 'fa' ? '🔙 بازگشت' : '🔙 Back', callback_data: 'back_to_main_panel' }
  ]);
...
// ============================================================
// 📦 تابع نمایش راهنمای شخصیت‌ها در Help
// ============================================================

async function showPersonalityHelp(cb: CallbackQuery, env: Env): Promise<void> {
  const session = await getOrCreateSession(cb.message!.chat, cb.from, env);
  const lang = session.language || 'fa';
  
  const text = lang === 'fa'
    ? `🎭 **راهنمای شخصیت‌های نوا**\n\n` +
      `ربات نوا دارای **۸ شخصیت** متفاوت است. هر شخصیت لحن، رفتار و تخصص خاص خودش را دارد.\n\n` +
      `**✨ شخصیت‌های دختر (صدای زن):**\n` +
      `🤖 **نوا** - دستیار هوشمند، مودب و مفید (پیش‌فرض)\n` +
      `🖤 **لیلیت** - اغواگر، بی‌پروا و جذاب\n` +
      `🌙 **آریا** - فیلسوف شورشی، عمیق و متفکر\n` +
      `🧠 **لونا** - مغز متفکر، منطقی و تحلیلی\n` +
      `✨ **زارا** - خلاق، هنری و الهام‌بخش\n\n` +
      `**✨ شخصیت‌های پسر (صدای مرد):**\n` +
      `💀 **سایفر** - هکر مرموز، تکنیکی و زیرک\n` +
      `🔥 **جکس** - آشوبگر، پرانرژی و شوخ\n` +
      `🪚 **صورت‌چرمی** - قاتل زنجیره‌ای خشن\n\n` +
      `💡 برای تغییر از منوی اصلی یا دستور /personality استفاده کنید.`
    : `🎭 **Nova Personalities Guide**\n\n` +
      `Nova has **8 unique personalities**. Each has its own tone, behavior, and expertise.\n\n` +
      `**✨ Female Personalities (Female voice):**\n` +
      `🤖 **Nova** - Smart, polite, helpful (default)\n` +
      `🖤 **Lilith** - Reckless seducer, bold\n` +
      `🌙 **Aria** - Rebel philosopher, deep\n` +
      `🧠 **Luna** - Deep thinker, logical\n` +
      `✨ **Zara** - Creative, artistic\n\n` +
      `**✨ Male Personalities (Male voice):**\n` +
      `💀 **Cipher** - Mysterious hacker\n` +
      `🔥 **Jax** - Chaos bringer, funny\n` +
      `🪚 **Leatherface** - Brutal killer\n\n` +
      `💡 Change from main menu or /personality command.`;
...
// ============================================================
// 📦 تابع تنظیمات مدل فعال (نسخه نهایی)
// ============================================================
...
async function handleExistingCallbacks(cb: CallbackQuery, session: ChatSession, env: Env, config: ReturnType<typeof createConfig>) {
  ...
  switch (data) {
    ...
    case 'set_personality_alien':      // 🆕 جدید
    case 'set_personality_victoria':   // 🆕 جدید
    {
...
// ============================================================
// 📦 تابع TRANSCRIBE VOICE با GEMINI
// ============================================================
...
async function handleSambanovaRequest(
  ...
  // ساخت پیام خطای ساده و کاربرپسند
...
async function handleCallbackQuery(cb: CallbackQuery, env: Env, config: ReturnType<typeof createConfig>): Promise<void> {
  ...
  try {
    ...
    // ادامه handleCallbackQuery از پارت 24
...
}  // <--- این } مال بسته شدن تابع handleCallbackQuery هست
...
case 'admin_page_next':
  ...
  break;
      ...
      default:
        ...
        if (data.startsWith('pollinations_model_')) {
        ...
        if (data.startsWith('select_model_')) {
          ...
          if (!selectedModel) {
            await answerCallbackQuery(cb.id, "❌ مدل یافت نشد", true);
            return;
          }
          
          session.engines[engine].modelIndex = modelIndex;
          await saveSessionWithLock(session, env, true);
          await answerCallbackQuery(cb.id, `✅ مدل ${selectedModel.name} انتخاب شد`, false);
          await showModelSelection(chat.id, cb.message.message_id, engine, false, env);
          break;
        }

        // اگر هیچکدام از موارد بالا نبود
        await answerCallbackQuery(cb.id, "❌ دستور ناشناخته", true);
        logger.warn(`Unknown callback data: ${data}`);
        break;
    }
  } catch (error) {
...
async function showGroupVIPPanel(chatId: number, messageId: number, env: Env): Promise<void> {
  try {
    ...
    logger.error("showGroupVIPPanel error:", error);
    await editMessageText(chatId, messageId, "❌ خطا در نمایش گروه‌های VIP");
  }
}
...
export default {
  ...
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext): Promise<void> {
    try {
      ...
      // 5. پردازش Broadcast Job (اگر وجود داشته باشد)
      try {
      ...
      // 6. پاکسازی خودکار سشن‌ها (هر ۶ ساعت)
      if (Math.random() < 0.01) { // ~1% شانس در هر اجرا
        await cleanupSessions(env);
...
// ============================================================
// 📦 تابع HANDLE START COMMAND (ادامه از پارت ۱۲)
// ============================================================
...
async function handleStartCommand(message: Message, env: Env) {
  ...
  // کاربر با session موجود - نمایش منوی اصلی
  await refreshUserCommands(chat.id, session);
  const welcomeText = t(session, isGroup ? 'welcome_group' : 'welcome_private', { name: from.first_name });
  const keyboard = getStartKeyboard(isGroup, session.language);
...
// ============================================================
// 📦 تابع HANDLE MODEL COMMAND (ادامه)
// ============================================================
...
// ============================================================
// 📦 تابع HANDLE SETVIP / UNSETVIP COMMANDS
// ============================================================
...
async function handleSetVipCommand(message: Message, args: string[], env: Env) {
  ...
  // اگر آرگومان دارد و عدد است => کاربر خاص
...
// ============================================================
// 📦 تابع HANDLE PROMPT COMMAND
// ============================================================

async function handlePromptCommand(message: Message, env: Env) {
  const { chat, from } = message;
  if (!from) return;

  const session = await getOrCreateSession(chat, from, env);
  await updatePromptMenu(chat.id, message.message_id, session);
}

// ============================================================
// 📦 تابع HANDLE LANGUAGE COMMAND (ادامه)
// ============================================================
...
// ============================================================
// 📦 تابع HANDLE ADMIN COMMAND (ادامه)
// ============================================================
...
// ============================================================
// 📦 تابع HANDLE KEYS COMMAND (ادامه از پارت ۸)
// ============================================================

async function handleKeysCommand(chatId: number, messageId: number | undefined, env: Env, isEdit = false) {
  // این تابع قبلاً در پارت ۸ کامل شده
  // فقط برای رفرنس اینجا قرار داده شده
  // کد کامل در پارت ۸ موجود است
}

// ============================================================
// 📦 تابع HANDLE BLOCKED COMMAND
// ============================================================

async function handleBlockedCommand(message: Message, env: Env): Promise<void> {
  const { chat, from } = message;
...
// ============================================================
// 📦 تابع HANDLE REBUILD COMMAND
// ============================================================

async function handleRebuildCommand(message: Message, env: Env): Promise<void> {
  const { chat, from } = message;
...
// ============================================================
// 📦 تابع HANDLE DELETE COMMAND (/del)
// ============================================================
...
// ============================================================
// 📦 تابع HANDLE REMOVE COMMAND (/remove)
// ============================================================
...
// ============================================================
// 📦 تابع پردازش پیام‌های غیردستوری در گروه
// ============================================================
...
async function processGroupMessage(message: Message, env: Env, config: ReturnType<typeof createConfig>) {
  ...
  // بررسی اینکه آیا باید در گروه پاسخ بده
  ...
  const isBlocked = await isUserBlocked(from.id, env);
  if (isBlocked && from.id !== config.BOT_OWNER_ID) {
    await sendMessage(chat.id, "🚫 **دسترسی مسدود**\n\nحساب شما مسدود است.\n\n📞 برای رفع مسدودیت با @Hamid_Ai_pro تماس بگیرید.", {
      reply_to_message_id: message.message_id
    });
    return;
  }
...
// ============================================================
// 📦 تابع HANDLE TEXT MESSAGE (نسخه نهایی و کامل)
// ============================================================
...
async function handleTextMessage(message: Message, env: Env, config: ReturnType<typeof createConfig>) {
  ...
  if (text.startsWith('/')) {
    ...
    // در گروه فقط اگر منشن شده باشه پاسخ بده
  ...
  // ===== پیام معمولی در چت خصوصی =====
  await handlePrivateTextMessage(message, env, config);
}

// ============================================================
// 📦 تابع HANDLE PRIVATE TEXT MESSAGE
// ============================================================
...
// ============================================================
// 📦 تابع HANDLE COMMAND (مسیریابی دستورات)
// ============================================================
...
async function handleCommand(command: string, args: string[], message: Message, env: Env) {
  ...
  // بررسی دسترسی در گروه برای برخی دستورات
  const isGroup = chat.type === "group" || chat.type === "supergroup";
...
// ============================================================
// 📦 تابع SEND PERSONALITY MENU (ادامه از پارت ۱۸)
// ============================================================
...
async function sendPersonalityMenu(chatId: number, messageId: number, session: ChatSession): Promise<void> {
  ...
  const text = lang === 'fa'
    ? `🎭 **انتخاب شخصیت ربات**\n\n` +
      `شخصیت فعلی: **${PERSONALITIES[currentPersonality]?.emoji || '🤖'} ${currentName}**\n\n` +
      `لطفاً شخصیت مورد نظر خود را انتخاب کنید:`
    : `🎭 **Select Bot Personality**\n\n` +
      `Current personality: **${PERSONALITIES[currentPersonality]?.emoji || '🤖'} ${currentName}**\n\n` +
      `Please select your desired personality:`;
  ...
  const keyboard = {
    inline_keyboard: [
      ...
      [
        ...
        { text: `${PERSONALITIES.leatherface.emoji} صورت‌چرمی`, callback_data: 'set_personality_leatherface' }
      ],
      [
...
// ============================================================
// 📦 توابع کمکی نهایی و تکمیل‌کننده‌ها
// ============================================================

// ============================================================
// 📦 تابع برای مدیریت خطاهای API Key
// ============================================================

async function handleAPIKeyError(error: Error, engine: string, env: Env): Promise<string> {
  const errorMsg = error.message.toLowerCase();
  
  if (errorMsg.includes('quota') || errorMsg.includes('429') || errorMsg.includes('rate limit')) {
    // غیرفعال کردن کلید فعلی و برگرداندن پیام مناسب
    if (engine === 'gemini' && config.GEMINI_KEYS.length > 0) {
      // کلید بعدی امتحان میشه
      return "⏳ سهمیه API تمام شده. در حال تلاش با کلید بعدی...";
    } else if (engine === 'sambanova' && config.SAMBANOVA_KEYS.length > 0) {
      return "⏳ سهمیه API لونا تمام شده. در حال تلاش با کلید بعدی...";
    }
    return "⏳ سهمیه API تمام شده. لطفاً مدل را تغییر دهید یا بعداً تلاش کنید.";
  }
  
  if (errorMsg.includes('blocked') || errorMsg.includes('safety') || errorMsg.includes('content filter')) {
    return "🛡️ محتوای درخواست شما توسط سیستم امنیتی رد شد. لطفاً متن را تغییر دهید.";
  }
  
  if (errorMsg.includes('timeout')) {
    return "⏱️ زمان پاسخگویی سرور تمام شد. لطفاً دوباره تلاش کنید.";
  }
  
  if (errorMsg.includes('network') || errorMsg.includes('fetch')) {
    return "🌐 مشکل در اتصال به شبکه. لطفاً اتصال اینترنت خود را بررسی کنید.";
  }
  
  return `❌ خطا در ${engine}: ${error.message.substring(0, 100)}`;
}

// ============================================================
// 📦 تابع برای نمایش وضعیت بات در گروه‌ها
// ============================================================
...
async function showBotStatusInGroup(message: Message, env: Env): Promise<void> {
  ...
  try {
    ...
    let statusText = `🤖 **وضعیت ربات در گروه**\n\n`;
    statusText += `• وضعیت: ${botMember.status === 'administrator' ? '✅ ادمین' : '👤 عضو معمولی'}\n`;
    
    if (botMember.status === 'administrator') {
      statusText += `• حقوق ادمین:\n`;
      statusText += `  - حذف پیام: ${botMember.can_delete_messages ? '✅' : '❌'}\n`;
      statusText += `  - بن کردن: ${botMember.can_restrict_members ? '✅' : '❌'}\n`;
      statusText += `  - افزودن عضو: ${botMember.can_invite_users ? '✅' : '❌'}\n`;
      statusText += `  - تغییر عنوان: ${botMember.can_change_info ? '✅' : '❌'}\n`;
    }
    ...
    await sendMessage(chat.id, "❌ **خطا در دریافت وضعیت ربات**\n\nلطفاً مطمئن شوید ربات در گروه عضو است.", {
      reply_to_message_id: message.message_id
    });
  }
}
...
// ============================================================
// 📦 تابع برای بررسی و بروزرسانی مدل‌ها
// ============================================================

async function refreshAllModels(env: Env): Promise<void> {
  try {
    logger.info("🔄 Starting model refresh...");
    
    // بروزرسانی مدل‌های SambaNova
    if (config.SAMBANOVA_KEYS.length > 0) {
      const sambaCache = await fetchAndCacheModels("sambanova", env);
      config.SAMBANOVA_MODELS = sambaCache.models.map(m => m.id);
      logger.info(`✅ SambaNova models updated: ${config.SAMBANOVA_MODELS.length} models`);
    }
    
    // بروزرسانی مدل‌های Pollinations
    const polliCache = await fetchAndCacheModels("pollinations", env);
    config.POLLINATIONS_MODELS = polliCache.models.map(m => m.id);
    logger.info(`✅ Pollinations models updated: ${config.POLLINATIONS_MODELS.length} models`);
    
    // بروزرسانی کش حافظه
    modelCache.set("models:sambanova", config.SAMBANOVA_MODELS.map(id => ({ id, name: id, type: 'text' as const })), 30 * 60 * 1000);
    modelCache.set("models:pollinations", config.POLLINATIONS_MODELS.map(id => ({ id, name: id, type: 'text' as const })), 30 * 60 * 1000);
    
    logger.info("✅ All models refreshed successfully");
...
// ============================================================
// 📦 تابع برای دریافت آمار دقیق کاربر
// ============================================================

async function getUserDetailedStats(userId: number, env: Env): Promise<UserStatistics | null> {
  try {
    const allUsers = await getAllUserStatistics(env);
...
// ============================================================
// 📦 تابع برای پاک کردن کش‌های مختلف
// ============================================================
...
// ============================================================
// 📦 تابع برای گرفتن نسخه و اطلاعات ربات
// ============================================================

function getBotInfo(): { version: string; name: string; username: string; owner: string } {
  return {
    version: BOT_VERSION,
    name: BOT_INFO?.first_name || "Nova AI",
    username: BOT_INFO?.username || "nova_bot",
    owner: "@Hamid_Ai_pro"
  };
}

// ============================================================
// 📦 تابع برای تبدیل Markdown به متن ساده (برای مواردی که Markdown پشتیبانی نمی‌شود)
// ============================================================
...
// ============================================================
// 📦 تابع برای بررسی سلامت اتصال به KV
// ============================================================

async function checkKVHealth(env: Env): Promise<{ status: 'ok' | 'error'; message: string }> {
  try {
    const testKey = `_health_${Date.now()}`;
    await env.SESSIONS.put(testKey, "ok");
    const result = await env.SESSIONS.get(testKey, "text");
    await env.SESSIONS.delete(testKey);
    
    if (result === "ok") {
      return { status: 'ok', message: 'KV connection is healthy' };
    } else {
      return { status: 'error', message: 'KV read/write test failed' };
    }
  } catch (error) {
    return { status: 'error', message: `KV error: ${error instanceof Error ? error.message : 'unknown'}` };
  }
}

// ============================================================
// 📦 EXPORT نهایی و تکمیل Worker
// ============================================================

// این قسمت در پارت‌های قبلی به عنوان export default تعریف شده
// اما برای تکمیل، مطمئن می‌شویم همه چیز درست است

console.log(`🚀 Nova Bot v${BOT_VERSION} loaded successfully!`);
console.log(`📊 Total lines: ~11732`);
...
console.log(`🎭 Personalities: 8 unique characters`);
