...
async function showMainPanel(chatId: number, messageId: number, session: ChatSession, env: Env): Promise<void> {
  ...
  const personality = session.activePersonality || 'nova';
  const personData = PERSONALITIES[personality];
  ...
  // نام کاربر
  const userName = session.userMemories.get(session.id)?.firstName || 'کاربر';
  
  // دریافت ترجمه
  const t = TRANSLATIONS[lang];
  
  // ✅ متن پنل - دقیقاً مثل عکس
  const text = lang === 'fa' 
    ? `${t.panel_title}\n${t.panel_version}\n\n` +
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `👤 **${t.panel_user} ${isVip ? t.panel_vip : t.panel_free}**\n` +
      `📛 ${userName}\n` +
      `🎭 ${t.panel_personality}: ${personData?.emoji || '🤖'} ${personData?.name_fa || 'نوا'}\n` +
      `━━━━━━━━━━━━━━━━━━━━\n\n` +
      `📊 **${t.panel_daily_usage}:**\n` +
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `💬 ${daily.messages}/${maxMessages} ${t.panel_messages}\n` +
      `🎤 ${daily.voicesSent}/${maxVoice} ${t.panel_voice}\n` +
      `🖼️ ${daily.imagesGenerated}/${maxImages} ${t.panel_images}\n` +
      `━━━━━━━━━━━━━━━━━━━━\n\n` +
      `**${t.panel_select}**`
    : `${t.panel_title}\n${t.panel_version}\n\n` +
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `👤 **${t.panel_user} ${isVip ? t.panel_vip : t.panel_free}**\n` +
      `📛 ${userName}\n` +
      `🎭 ${t.panel_personality}: ${personData?.emoji || '🤖'} ${personData?.name_en || 'Nova'}\n` +
      `━━━━━━━━━━━━━━━━━━━━\n\n` +
      `📊 **${t.panel_daily_usage}:**\n` +
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `💬 ${daily.messages}/${maxMessages} ${t.panel_messages}\n` +
  ...
  // ✅ کیبورد - دقیقاً مثل عکس با ۲ دکمه در هر ردیف
  const keyboard = {
    inline_keyboard: [
      // ردیف 1: تغییر شخصیت | گفتگوی جدید
      [
        { text: t.btn_change_personality, callback_data: "personality_menu" },
        { text: t.btn_new_chat, callback_data: "new_chat_confirm" }
      ],
      // ردیف 2: پرامپت سفارشی | تغییر زبان
      [
        { text: t.btn_custom_prompt, callback_data: "custom_prompt_menu" },
        { text: t.btn_change_language, callback_data: "language_menu" }
      ],
      // ردیف 3: ارتقا به VIP | راهنما
      [
        { text: isVip ? "👑 VIP فعال" : t.btn_upgrade_vip, callback_data: "upgrade_vip" },
        { text: t.btn_help, callback_data: "open_help" }
      ],
      // ردیف 4: بستن منو (تک دکمه در وسط)
      [
        { text: t.btn_close_menu, callback_data: "close_panel" }
      ]
    ]
  };
...
async function handleGroupModeSwitch(session: ChatSession, mode: string, cb: CallbackQuery, env: Env): Promise<void> {
  const modes: Record<string, { mode: 'mention_only'; label: string }> = {
...
// ============================================================
// 🚀 NOVA BOT V2.0 - پارت ۱۲: پنل ادمین کامل (Admin Panel)
// ============================================================
...
async function updateAdminPanel(chatId: number, messageId: number, env: Env) {
  ...
  text += `📊 مرتب‌سازی: `;
  text += state.sortBy === 'new' ? '🆕 جدیدترین' : 
           state.sortBy === 'active' ? '⚡ فعال‌ترین' : '💬 پرپیام‌ترین';
  text += `\n━━━━━━━━━━━━━━━━━━━━\n\n`;
  
  // نمایش کاربران در صفحه جاری
  if (pageUsers.length === 0) {
    text += `📭 **هیچ کاربری یافت نشد**`;
  } else {
    pageUsers.forEach((user, idx) => {
      const num = startIdx + idx + 1;
      const escapedName = user.firstName || 'نامشخص';
